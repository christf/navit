...
/** Minimum absolute delta for a turn of "normal" strength (which is always just announced as "turn left/right" even
 * when strength is required). Maneuvers whose absolute delta is less than this will be announced as "turn easily
 * left/right" when strength is required. */
static int turn_2_limit = 45;

/** Minimum absolute delta for a sharp turn.
 * Maneuvers whose absolute delta is equal to or greater than this will be announced as "turn sharply left/right" when
 ...
 * Note that, depending on other conditions, even maneuvers whose delta exceeds the threshold may still be announced as
 * (sharp) turns. */
...
struct navigation {
    ...
    char *speech_text;              /**< Speech text selected for the current announcement event */
    char **speech_alt;              /**< NULL-terminated fallback alternatives for {@code speech_text} */
    /* Configuration snapshot used when rendering announcement tables. If any of these changes,
     * the tables must be regenerated (see navigation_announcements_check_profile()). */
    int announce_imperial;         /**< 'imperial' setting at announcement render time */
    int announce_vocab_distances;  /**< 'vocabulary_distances' setting at announcement render time */
    int announce_vocab_name;       /**< 'vocabulary_name' setting at announcement render time */
    int announce_vocab_systematic; /**< 'vocabulary_name_systematic' setting at announcement render time */
};
...
struct navigation_maneuver {
    enum item_type type; /**< The type of maneuver to perform. Any {@code nav_*} item is permitted here, with one
                              exception: merge or exit maneuvers are indicated by the {@code merge_or_exit} member. The
                              {@code item_type} for such maneuvers should be a turn instruction in cases where the
                              maneuver is ambiguous, or
                              {@code nav_none} for cases in which we would expect the driver to perform this maneuver
                              even without being instructed to do so. **/
    ...
    int merge_or_exit; /**< Whether we are merging into or exiting from a motorway_like road or we are at an interchange
                        */
    int is_complex_t_junction; /**< Whether we are coming from the "stem" of a T junction whose "bar" is a
                                    dual-carriageway road and crossing the opposite lane of the "bar" first (i.e.
                                    turning left in countries that drive on the right, or turning right in countries
                                    that drive on the left). For these maneuvers {@code num_options} is 1 (which means
                                    we normally wouldn't announce the maneuver) but drivers would expect an
                                    announcement in such cases. */
    ...
    int num_new_motorways; /**< Number of permitted candidate ways that are motorway-like.
                                Numbers greater then one should be interpreted as "more than one", not as a precise
                                number. */
    ...
    int num_similar_ways;  /**< Number of candidate ways (including the route) that have a {@code maneuver_category()}
                                similar  to {@code old_cat}. See {@code maneuver_required2()} for definition of
                              "similar".
                            */
    ...
    int is_unambiguous;    /**< Whether the maneuver is unambiguous. A maneuver is unambiguous if, despite
                                ...
                                route stays on the main road and goes straight, while all other candidate ways
 ...
 */
/**
 * @brief Speech texts for one maneuver, rendered ahead of time.
 *
 * All strings which may ever be spoken for a command are generated when the maneuvers are built,
 * i.e. right at the beginning of routing. Which of them is actually spoken (and whether it is
 * spoken at all) remains a run-time decision.
 *
 * The strings are owned by this structure and must be freed with navigation_announcements_free().
 */
struct navigation_announcements {
    int valid;          /**< Nonzero once the texts have been generated */
    char *soon;         /**< Text for {@code level_soon} ("Turn left soon"), NULL if not generated */
    char *meters;       /**< Text for {@code level_meters}, with the distance set to the configured threshold */
    char *now[2];       /**< Text for {@code level_now}, indexed by the streetname_told state at speech time */
    char *connect;      /**< Text for {@code level_connect} of THIS command, used when the preceding
                             announcement is concatenated with it ("then turn right") */
    char *follow[3];    /**< Texts for {@code level_follow}. These embed a distance, which is only
                             known at speech time; the three entries cover the rounded distance at
                             maneuver build time and its neighboring distance buckets. */
    int follow_dist[3]; /**< The distances (already rounded) embedded in {@code follow[]}; parallel array */
    int follow_count;   /**< Number of valid entries in {@code follow[]} / {@code follow_dist[]} (0-3) */
};
...
struct navigation_command {
    ...
    int roundabout_delta; /**< if we are leaving a roundabout, effective bearing change (between entry and exit) with
    ...
    struct navigation_announcements announcements; /**< Pre-rendered speech texts for this maneuver */
};
...
static int is_same_street2(char *old_name, char *old_name_systematic, char *new_name, char *new_name_systematic) {
    if (old_name && new_name && !strcmp(old_name, new_name)) {
        dbg(lvl_debug, "is_same_street: '%s' '%s' vs '%s' '%s' yes (1.)", old_name_systematic, new_name_systematic,
            old_name, new_name);
        return 1;
    }
    if (old_name_systematic && new_name_systematic && !strcmp(old_name_systematic, new_name_systematic)) {
        dbg(lvl_debug, "is_same_street: '%s' '%s' vs '%s' '%s' yes (2.)", old_name_systematic, new_name_systematic,
            old_name, new_name);
        return 1;
    }
...
static int set_highrank(struct street_destination *street_destination, struct navigation_command *command) {
    ...
    if (command->next) {
        ...
        if (next_command->itm->way.destination)
            ...
            future_street_destination = NULL;
    }
 ...
 *         command items so that the destination name has a relevance over several announcements. If there is no
 *         'winner' the entry is selected that is at top of the destination.
 */
...
#define MAX_DESTINATIONS 10                  /* limits the number of destination entries to investigate */
    ...
    if (current_command->itm->way.destination) {
        ...
        if (current_command->next) {
            /* loop over every destination of the current command, as far as there are not more than MAX_DESTINATIONS
             * entries. */
...
static void navigation_announcements_free(struct navigation_announcements *ann);
static void navigation_build_announcements(struct navigation *this_);
...
int navigation_set_announce(struct navigation *this_, enum item_type type, int *level) {
    ...
    /* The thresholds are baked into the pre-rendered announcement texts (trigger levels and
     * embedded distances), so they must be regenerated. */
    if (this_->cmd_first)
        navigation_build_announcements(this_);
    return 1;
}
...
static enum announcement_level navigation_get_announce_level_cmd(struct navigation *this_, struct navigation_itm *itm,
                                                                 struct navigation_command *cmd, int distance) {
...
static char *get_distance_str(struct navigation *nav, int dist_meters, enum attr_type type, int is_length) {
    ...
    if (imperial) {
        ...
        if ((dist_meters < METERS_TO_MILES) && (vocabulary > 0)) {
            /* values smaller than one need extra treatment for one decimal place. For reduced vocabulary it shall
             * remain 'one'. */
        ...
        if (is_length)
            return g_strdup_printf(navit_nls_ngettext("one kilometer", "%d kilometers", dist_meters / 1000),
                                   dist_meters / 1000);
        else
            return g_strdup_printf(navit_nls_ngettext("in one kilometer", "in %d kilometers", dist_meters / 1000),
                                   dist_meters / 1000);
    }
}
...
/**
 * @brief Returns the maximum delta between a reference bearing and any segment of a given way, up to a given distance
 * from its start
 *
 * The return value is the maximum delta (in terms of absolute value), but the sign is preserved. If the maximum delta
 * is encountered more than once but with different signs, {@code dir} controls which of the two values is returned:
 * <ul>
...
static void navigation_destroy_itms_cmds(struct navigation *this_, struct navigation_itm *end) {
    ...
    dbg(lvl_info, "enter this_=%p this_->first=%p this_->cmd_first=%p end=%p", this_, this_->first, this_->cmd_first,
        end);
    if (this_->cmd_first)
...
static struct navigation_itm *navigation_itm_new(struct navigation *this_, struct item *routeitem) {
    ...
    if (routeitem) {
         ...
         */
        if (ret->way.next
            && ((streetitem->type == type_ramp) || (streetitem->type == type_highway_land)
                ...
                if (rampitem->type == type_highway_exit && item_coord_get(rampitem, &exitcoord, 1)
                    ...
                    while (item_attr_get(rampitem, attr_any, &attr)) {
                        ...
                        if (attr.type == attr_exit_to) {
                             ...
                             */
                            if (attr.u.str && !ret->way.destination && (ret->way.item.type == type_ramp)
                                && (this_->last) && (!(this_->last->way.item.type == type_ramp))) {
                                char *destination_raw;
...
static int is_way_allowed(struct navigation *nav, struct navigation_way *way, int mode) {
    ...
    if (mode)
        return ((way->flags
                 & (way->dir >= 0 ? nav->vehicleprofile->flags_forward_mask : nav->vehicleprofile->flags_reverse_mask))
                == nav->vehicleprofile->flags);
    else
...
static int maneuver_required2(struct navigation *nav, struct navigation_itm *old, struct navigation_itm *new,
    ...
    int dc;   /* if new and another way are within +/-min_turn_limit and on the same side, bearing difference for the
                 other way; else d */
    ...
    *maneuver = NULL;
    ...
    m.is_same_street =
        is_same_street2(old->way.name, old->way.name_systematic, new->way.name, new->way.name_systematic);
    ...
    if (!r) {
        ...
        while (w) {
            ...
            if ((w->angle2 != new->way.angle2) || (w == &(new->way))) {
                ...
                if (is_way_allowed(nav, w, 1)) {
                    dbg(lvl_debug, "- Examining allowed way: %s %s %s, delta=%i", item_to_name(w->item.type),
                        w->name_systematic, w->name, dw);
                    m.num_options++;
                    ...
                    if ((m.num_new_motorways < 2) && is_motorway_like(w, 0)) {
                        ...
                        if ((w == &(new->way)) || new->way.exit_ref || new->way.exit_label || w->exit_ref
                            || w->exit_label || !old->way.name_systematic || !new->way.name_systematic
                            || !w->name_systematic || strcmp(old->way.name_systematic, new->way.name_systematic)
                    ...
                    if (w != &(new->way)) {
                        ...
                        if (is_motorway_like(w, 0) && is_motorway_like(&(old->way), 0)
                            && new->way.item.type == type_ramp) {
                            if (dw < m.delta)
                                motorways_left++;
                            else
                                motorways_right++;
                        ...
                        /* If almost-straight ways are present, the maneuver is ambiguous. We are counting only ways
                         * having a nonzero maneuver category (street_0 or higher), excluding ramps, service roads and
                         * anything closed to motorized traffic. Exceptions apply when the new way itself has a maneuver
                         * category of 0. Note that this is in addition for the dlim calculations we do further below,
                         * as they fail to catch some ambiguous cases for very low deltas. */
                        if ((dw > -min_turn_limit) && (dw < min_turn_limit)
                            && ((maneuver_category(w->item.type) != 0) || (maneuver_category(new->way.item.type) == 0)))
                            m.is_unambiguous = 0;
                         ...
                         * - the other road is a ramp or service road (they are sometimes tagged with the name of the
                         * motorway) The second one is really a workaround for bad tagging practice in OSM. Since
                         * entering a ramp always creates a maneuver, we don't expect the workaround to have any
                         * unwanted side effects.
                         */
                        if (m.is_same_street
                            && is_same_street2(old->way.name, old->way.name_systematic, w->name, w->name_systematic)
                            && (!is_motorway_like(&(old->way), 0)
                                || (!is_motorway_like(w, 0) && w->item.type != type_ramp))
                            && is_way_allowed(nav, w, 2))
                            m.is_same_street = 0;
                        /* If the route category changes to a lower one but another road has the same route category as
                         * old, it is not clear which of the two the driver would perceive as the "same street", hence
                         * reset is_same_street */
                ...
                if (w->flags & AF_ONEWAYMASK) {
                    ...
                    if (is_same_street2(new->way.name, new->way.name_systematic, w->name, w->name_systematic))
                        ...
                        through_segments++;
                    else if ((!through_segment2
                              && (abs(angle_delta(angle_opposite(w->angle2), new->way.angle2)) < min_turn_limit))
                             || (through_segment2
                                 && (abs(angle_delta(angle_opposite(w->angle2), new->way.angle2))
                                     < abs(angle_delta(angle_opposite(through_segment2->angle2), new->way.angle2))))) {
                        /* also count near-straight continuations of new (the through road may change names at the
                         * junction) */
                        if (!through_segment2)
                            through_segments++;
                        through_segment2 = w;
        ...
        if ((abs(m.delta) >= min_turn_limit) && (through_segments >= 2)) {
            ...
            while (ni && (hist_through_segments == 0) && (hist_dist <= junction_limit)) {
                /* TODO: shouldn't the condition be (hist_through_segments < 2) to catch through segments that are
                 * slightly offset?? */
                struct navigation_way *w = ni->way.next;
                while (w) {
                    if (is_same_street2(new->way.name, new->way.name_systematic, w->name, w->name_systematic)
                        || (through_segment2
                            && is_same_street2(through_segment2->name, through_segment2->name_systematic, w->name,
                                               w->name_systematic)))
    ...
    if (!r) {
        ...
        r = g_strdup_printf("%s: d %d left %d right %d dlim=%d cat old:%d new:%d max:%d unambiguous=%d same_street=%d",
                            ret == 1 ? "yes" : "no", m.delta, m.left, m.right, dlim, m.old_cat, m.new_cat, m.max_cat,
                            m.is_unambiguous, m.is_same_street);
#endif
    }
    ...
    if (r)
        dbg(lvl_debug, "%s %s %s -> %s %s %s: %s, delta=%i, merge_or_exit=%i", item_to_name(old->way.item.type),
            old->way.name_systematic, old->way.name, item_to_name(new->way.item.type), new->way.name_systematic,
            new->way.name, r, m.delta, m.merge_or_exit);
    return ret;
}
 ...
 *
 * Calculated delta results are constrained to -180 <= delta <= +180 degrees. Some heuristics, however, require values
 * outside that range, e.g. -190 rather than 170 degrees. Given a reference with approximately correct orientation, this
 * function corrects that delta to match the reference as closely as possible. This is done by adding or subtracting 360
 * degrees as needed.
 *
 ...
 * @return The adjusted delta, which is numerically within +/-180 degrees of the reference. If {@code delta} or {@code
 * reference} are outside of their specified range, the result is undefined.
 */
 ...
 *
 * Combining {@code delta1} and {@code delta2}, calculating error estimates for each and using a weighted average
 * between the two delta estimates gives better results in many cases but fails for certain road layouts - namely, when
 * corresponding approach roads are connected by more than one roundabout segment, or when additional ways connect to an
 * approach road. These cases break error calculation and thus weight distribution.
 *
 * Project HighFive introduces a new approach, which compares bearings of ways leading towards and away from the
 * roundabout not immediately at entry and exit but at a certain distance from the roundabout, which is roughly
 * proportional to the circumference of the roundabout. Circumference is estimated using the arithmetic mean value of
 * {@code delta1} and
 * {@code delta2}. This approach has produced the best results in tests. In code it is referred to as {@code delta3}.
 *
 * @param this_ The navigation object
 * @param cmd A {@code struct navigation_cmd}, whose {@code delta} and {@code maneuver} members must be set prior to
...
static void navigation_analyze_roundabout(struct navigation *this_, struct navigation_command *cmd,
    ...
    if (w) {
        ...
        if (itm2 && w2) {
            ...
            dbg(lvl_debug, "entry %d median from %d (%d), %d", entry_tangent, angle_opposite(itm2->way.angle2),
                itm2->way.angle2, itm2->way.next->angle2);
        } else {
        ...
        if (itm2->prev) {
            ...
            if (central_angle)
                ...
                dbg(lvl_error,
                    "central_angle in roundabout_length computation lead to divide by zero (delta1 = %d, delta2 = %d, "
                    "cmd->delta  = %d, dtsir = %d, len = %d)",
                    delta1, delta2, cmd->delta, dtsir, len);
                roundabout_length = len;
            }
            dbg(lvl_debug, "roundabout_length = %dm (for central_angle = %d degrees)", roundabout_length,
                central_angle);

            /* in the case of separate carriageways, approach roads become hard to identify, thus we keep a cap on
             * distance. Currently this is at most half the length of the roundabout, which has worked well in tests but
             * can be tweaked to further improve results. */
            ...
            while (itm3->prev) {
                ...
                w2 = itm3->way.next;
                while (w2) {
                    /* Stop examining ways at a turn maneuver (more than one way allowed and route does not follow
                     * straightest path) */
                    if (is_way_allowed(this_, w2, 0)
                        && (abs(angle_delta(angle_opposite(w2->angle2), itm3->way.angle2))
                            <= abs(angle_delta(itm3->prev->angle_end, itm3->way.angle2)))) {
            ...
            while (itm3->next) {
                ...
                while (w2) {
                     ...
                     * straightest path) */
                    if (is_way_allowed(this_, w2, 0)
                        && (abs(angle_delta(itm3->angle_end, w2->angle2))
                            <= abs(angle_delta(itm3->angle_end, itm3->next->way.angle2)))) {
                        /* FIXME: comparing angles probably does not work well for near-equal angles */
                        abort = 1;
                        break;
                    }
 ...
 * initialize a zeroed-out {@code struct navigation_maneuver} and set its {@code type} member to {@code
 * type_nav_destination} prior to calling this function.
 *
...
static struct navigation_command *command_new(struct navigation *this_, struct navigation_itm *itm,
    ...
    int turn_no_of_route_way =
        0; /* The number of the route way of all ways that turn to the same direction.
     ...
     * nav_none                    (default, change wherever we encounter it – unless the maneuver is a merge, which has
     *                             only merge_or_exit)
    ...
    if (ret->maneuver->type != type_nav_destination) {
        /* FIXME: this will not catch cases in which entry and exit share the same node and we just *touch* the
         * roundabout */
        if (itm && itm->prev && !(itm->way.flags & AF_ROUNDABOUT) && (itm->prev->way.flags & AF_ROUNDABOUT)) {
            ...
            if (abs(ret->delta) >= min_turn_limit) {
                ...
                /* Strengthening criterion: If there are more ways in the same direction, in which the vehicle can turn,
                   the announcement shall be more precise. I.e. the strengthening is dependent on which of the possible
                   ways the route turn shall be. So, with the selection of one of the possible ways a certain turn angle
                   pattern becomes active. Second criterion: the turn angle of the route way defines the strengthening
                   of the announcement according to the pattern. */
                w = itm->next->way.next;
                    ...
                    while (w) {
                        if (angle_delta(w->angle2, itm->angle_end) < -min_turn_limit) {
                            more_ways_for_strength++; /* there is an additional way that also turns left.
                                                         Note: the route turn is not contained Left means more than
                                                         min_turn_limit, less is straight on */
                 ...
                 * If there's another way on one side of the route within 2 * min_turn_limit (not both - the expression
                 * below is a logical XOR), the maneuver is "keep left" or "keep right", else it is "go straight". Note
                 * that neighbors are not necessarily straight. The boundary may need some tweaking, (2 *
                 * min_turn_limit) may not be ideal but it's a first shot which ensures that other straight ways will
                 * always fulfill the neighbor criteria. */
...
static void navigation_cmd_free(struct navigation *this_) {
    ...
    while (cmd) {
        ...
        if (cmd->maneuver)
            g_free(cmd->maneuver);
        navigation_announcements_free(&cmd->announcements);
        g_free(cmd);
        cmd = next;
    }
...
static void make_maneuvers(struct navigation *this_, struct route *route) {
    ...
    /* Maneuver geometry is final now: render every speech text we may ever need, so that the
     * spoken instruction for each maneuver is known from this point on. */
    navigation_build_announcements(this_);
}
...
static char *navigation_item_destination(struct navigation *nav, struct navigation_command *cmd,
                                         struct navigation_itm *next, char *prefix) {
    ...
    /* On motorway links don't announce the name of the ramp as this is done by name_systematic and the
     * street_destination. */
    ...
    if (cmd->maneuver && cmd->maneuver->type && (cmd->maneuver->merge_or_exit & mex_merge)) {
        if (name_systematic)
            /* TRANSLATORS: Arguments: 1: Prefix (Space if required) 2: Systematic Street Name 3: Separator (Space if
             * required), 4: Street Name */
            ...
            case unknown:
                ...
                break;
            case masculine:
                /* TRANSLATORS: Arguments: 1: Prefix (Space if required) 2: Street Name. Masculine form. The stuff after
                 * | doesn't have to be included */
                ret = g_strdup_printf(_("%1$sonto %2$s|masculine form"), prefix, name1);
                break;
            case feminine:
                /* TRANSLATORS: Arguments: 1: Prefix (Space if required) 2: Street Name. Feminine form. The stuff after
                 * | doesn't have to be included */
                ret = g_strdup_printf(_("%1$sonto %2$s|feminine form"), prefix, name1);
                break;
            case neuter:
                /* TRANSLATORS: Arguments: 1: Prefix (Space if required) 2: Street Name. Neuter form. The stuff after |
                 * doesn't have to be included */
            ...
            ret = g_strdup("");
               ...
               && (!cmd->maneuver || (cmd->maneuver->merge_or_exit != mex_interchange))) {
        itm_pre = itm->prev;
        ...
        if (((itm_post) && is_motorway_like(&(itm_post->way), 0))
            ...
            ret = g_strdup_printf(
                "%s%s", prefix, _("onto the motorway ramp")); /* This is only announced when there is no additional info
                                                                 about the ramp and the ramp leads to a motorway. */
        else
            ret = g_strdup("");
        ...
        switch (gender) {
        case unknown:
            /* TRANSLATORS: Arguments: 1: Prefix (Space if required) 2: Street Name 3: Separator (Space if required), 4:
             * Systematic Street Name */
            ret = g_strdup_printf(_("%sinto %s%s%s"), prefix, name, sep, name2);
            break;
        case masculine:
            /* TRANSLATORS: Arguments: 1: Prefix (Space if required) 2: Street Name 3: Separator (Space if required), 4:
             * Systematic Street Name. Masculine form. The stuff after | doesn't have to be included */
            ret = g_strdup_printf(_("%sinto %s%s%s|masculine form"), prefix, name1, sep, name2);
            break;
        case feminine:
            /* TRANSLATORS: Arguments: 1: Prefix (Space if required) 2: Street Name 3: Separator (Space if required), 4:
             * Systematic Street Name. Feminine form. The stuff after | doesn't have to be included */
            ret = g_strdup_printf(_("%sinto %s%s%s|feminine form"), prefix, name1, sep, name2);
            break;
        case neuter:
            /* TRANSLATORS: Arguments: 1: Prefix (Space if required) 2: Street Name 3: Separator (Space if required), 4:
             * Systematic Street Name. Neuter form. The stuff after | doesn't have to be included */
 ...
 * in..., then turn right"
 * @param told_state The value of {@code streetname_told} to assume for this rendering. Only used for
 * speech announcements at level_now, where it determines whether the street name is included. This function
 * never modifies any state; maintaining {@code streetname_told} is the caller's responsibility.
 */
static char *show_maneuver(struct navigation *nav, struct navigation_itm *itm, struct navigation_command *cmd,
                           enum attr_type type, enum announcement_level level, int distance_override, int told_state) {
    ...
    if (street_destination)
        if (level == level_connect) {
            /* for the connected announcement suppress the destination, if this is the same as in the first part of the
             * announcement */
            if (cmd->prev) {
                char *street_destination_previous = select_announced_destinations(cmd->prev);
                if (street_destination_previous
                    && strstr(street_destination_previous,
                              street_destination)) { /* doesn't have any new information? */
        ...
        if (cmd->itm->next) {
            if (type == attr_navigation_speech) {
                ...
                if (level == level_meters) {
                    ...
                    if (told_state == 0) /* we are right at the intersection */
                        tellstreetname = 1;
        ...
        if (!(cmd->maneuver->merge_or_exit == mex_none)) {
            ...
            case mex_merge_right:
                if (cmd->maneuver->merge_or_exit == mex_merge_right) {
                    if (level == level_connect)
                        /* TRANSLATORS: the arg. is the phrase 'onto ...'. Right merge, the stuff after | doesn't have
                         * to be included. */
                        instruction = g_strdup_printf(_("then merge%1$s|right"), destination);
                    else
                        /* TRANSLATORS: the first arg. is distance, the second is the phrase 'onto ...'. Right merge,
                         * the stuff after | doesn't have to be included. */
                        instruction = g_strdup_printf(_("Merge %1$s%2$s|right"), d, destination);
                } else {
                    if (level == level_connect)
                        /* TRANSLATORS: the arg. is the phrase 'onto ...'. Left merge, the stuff after | doesn't have to
                         * be included. */
                        instruction = g_strdup_printf(_("then merge%1$s|left"), destination);
                    else
                        /* TRANSLATORS: the first arg. is distance, the second is the phrase 'onto ...'. Left merge, the
                         * stuff after | doesn't have to be included. */
                        instruction = g_strdup_printf(_("Merge %1$s%2$s|left"), d, destination);
                }
                break;
            case mex_exit_left:
            case mex_exit_right:
                direction = (cmd->maneuver->merge_or_exit == mex_exit_left) ? g_strdup(_("on your left"))
                                                                            : g_strdup(_("on your right"));
                if (exit_announce)
                    /* TRANSLATORS: the first arg. is exit ref and/or name, the second is the direction of exit and the
                     * third is distance */
                    instruction = g_strdup_printf(_("Take exit %1$s %2$s %3$s"), exit_announce, direction, d);
                else
                    /* TRANSLATORS: the first arg. is the direction of exit, the second is distance, the third is
                     * destination */
            ...
            if (!instruction && exit_announce) {
                ...
                at = g_strdup_printf(" %1$s %2$s",
                                     cmd->maneuver->merge_or_exit == mex_interchange ? (_("at interchange"))
                                                                                     : (_("at exit")),
        ...
        if (!instruction) {
            ...
            case type_nav_straight:
                if (level == level_connect)
                    ...
                    /* TRANSLATORS: the first arg. is distance, the second is where to do the maneuver, the third is
                     * destination */
            ...
            case type_nav_left_3:
                ...
                case type_nav_right_3:
                    ...
                    if (level == level_now)
                        skip_roads = count_possible_turns(nav, cmd->prev ? cmd->prev->itm : itm, cmd->itm, 90);
                    else if (level == level_connect)
                ...
                if (skip_roads > 0) {
                    if (skip_roads < 6) {
                        if (level == level_connect)
                            instruction = g_strdup_printf(_("then take the %1$s road to the %2$s"),
                                                          get_count_str(skip_roads + 1), direction);
                        else
                            instruction = g_strdup_printf(_("Take the %1$s road to the %2$s"),
                                                          get_count_str(skip_roads + 1), direction);
                ...
                if (skip_roads <= 0) {
                    /* TRANSLATORS: the first arg. is strength, the second is direction, the third is distance, the
                     * fourth is destination  */
    ...
    case level_follow:
        ...
        break;
    case level_soon:
        if (at) /* 'at' contains interchange or exit information that shall not be combined with
                   street_destination_announce */
    ...
    dbg(lvl_debug, "instruction: %s", ret);

    return ret;
}

/**
 * @brief Frees all speech texts owned by a {@code struct navigation_announcements}.
 *
 * @param ann The structure whose texts should be freed. Members are reset to safe values.
 */
static void navigation_announcements_free(struct navigation_announcements *ann) {
    int i;
    g_free(ann->soon);
    ...
    g_free(ann->now[1]);
    g_free(ann->connect);
    for (i = 0; i < ann->follow_count; i++)
        g_free(ann->follow[i]);
    memset(ann, 0, sizeof(*ann));
}

/**
 * @brief Size of the distance bucket around {@code dist} used by {@code round_distance()}.
 *
 * Used to derive the neighboring distance buckets when pre-rendering level_follow texts.
 */
static int announce_bucket_step(int dist) {
    if (dist < 100)
        return 10;
    if (dist < 250)
        return 25;
...
/**
 * @brief Renders all speech texts which may ever be spoken for one command.
 *
 * This is the plan-time half of announcement generation: everything that depends only on the
 * route geometry, map data and configuration is resolved here, at maneuver build time. The
 * resulting strings are stored in {@code cmd->announcements} and are later selected by
 * {@code navigation_get_speech_text()} as the vehicle approaches.
 *
 * Distances embedded in level_meters texts use the configured announcement threshold for the
 * street type of the maneuver, so they match what would be spoken when the vehicle enters the
 * respective zone. For level_now both streetname_told variants are generated because whether the
 * "in X meters" announcement was actually spoken (and thus already named the street) is only
 * known at run time.
 *
 * @param this_ The navigation object
 * @param cmd The command to render announcements for
 */
static void navigation_cmd_build_announcements(struct navigation *this_, struct navigation_command *cmd) {
    struct navigation_announcements *ann = &cmd->announcements;
    /* Deterministic reference item in place of the vehicle position: for maneuvers following a
     * previous one this is the previous maneuver's item; otherwise it is the approach segment.
     * It only matters where show_maneuver() needs a positional reference without using the
     * resulting distance (or where the distance is overridden anyway). */
    struct navigation_itm *ref_itm;
    int meters_override = -1;

    navigation_announcements_free(ann);

    if (!cmd->itm)
        return;

    ref_itm = cmd->prev ? cmd->prev->itm : (cmd->itm->prev ? cmd->itm->prev : cmd->itm);
    if (cmd->itm->way.item.type >= route_item_first && cmd->itm->way.item.type <= route_item_last)
        meters_override = this_->announce[cmd->itm->way.item.type - route_item_first][1];

    ann->soon = show_maneuver(this_, ref_itm, cmd, attr_navigation_speech, level_soon, -1, 0);
    ann->meters = show_maneuver(this_, ref_itm, cmd, attr_navigation_speech, level_meters, meters_override, 0);
    ann->now[0] = show_maneuver(this_, ref_itm, cmd, attr_navigation_speech, level_now, -1, 0);
    ann->now[1] = show_maneuver(this_, ref_itm, cmd, attr_navigation_speech, level_now, -1, 1);
    if (cmd->prev)
        ann->connect = show_maneuver(this_, cmd->prev->itm, cmd, attr_navigation_speech, level_connect, -1, 0);

    /* level_follow texts embed a distance which at speech time is the vehicle's current
     * distance to this maneuver. That value cannot be known in advance exactly, but it is very
     * close to the build-time distance (the announcement fires on the first update after
     * routing). Pre-render the rounded build-time distance and both neighboring buckets so the
     * spoken string is always one of the pre-synthesized ones. */
    {
        int idx = cmd->itm->way.item.type - route_item_first;
        int soon_thresh = (idx >= 0 && idx <= route_item_last - route_item_first) ? this_->announce[idx][2] : -1;
        int dist = ref_itm->dest_length - cmd->itm->dest_length;
        int d0 = round_distance(dist);
        int step = announce_bucket_step(d0);
        int cands[3];
        int n_cands = 0, i, j;
        cands[n_cands++] = round_distance(d0 - step);
        cands[n_cands++] = d0;
        cands[n_cands++] = round_distance(d0 + step);
        for (i = 0; i < n_cands; i++) {
            int dup = 0;
            /* Skip buckets below the soon threshold: level_follow can never fire there. */
            if (soon_thresh >= 0 && cands[i] <= soon_thresh)
                continue;
            for (j = 0; j < i; j++)
                if (!dup && cands[j] == cands[i])
                    dup = 1;
            if (dup || ann->follow_count >= 3)
                continue;
            ann->follow_dist[ann->follow_count] = cands[i];
            ann->follow[ann->follow_count] =
                show_maneuver(this_, ref_itm, cmd, attr_navigation_speech, level_follow, cands[i], 0);
            ann->follow_count++;
        }
    }
...
/**
 * @brief (Re)builds pre-rendered speech announcements for all commands.
 *
 * Called whenever maneuvers have been (re)built or announcement-relevant configuration has
 * changed. Also snapshots the configuration values the rendered texts depend on, so that
 * {@code navigation_announcements_check_profile()} can detect later changes.
 *
 * @param this_ The navigation object
 */
static void navigation_build_announcements(struct navigation *this_) {
    ...
    this_->announce_imperial = 0;
    this_->announce_vocab_distances = 1;
    this_->announce_vocab_name = 1;
    this_->announce_vocab_systematic = 1;
    if (this_->navit && navit_get_attr(this_->navit, attr_imperial, &attr, NULL))
        this_->announce_imperial = attr.u.num;
    if (this_->speech) {
        if (speech_get_attr(this_->speech, attr_vocabulary_distances, &attr, NULL))
            this_->announce_vocab_distances = attr.u.num;
        if (speech_get_attr(this_->speech, attr_vocabulary_name, &attr, NULL))
            this_->announce_vocab_name = attr.u.num;
        if (speech_get_attr(this_->speech, attr_vocabulary_name_systematic, &attr, NULL))
            this_->announce_vocab_systematic = attr.u.num;
    }

    for (cmd = this_->cmd_first; cmd; cmd = cmd->next)
        navigation_cmd_build_announcements(this_, cmd);
}

/**
 * @brief Checks whether pre-rendered announcements were built under a different configuration.
 *
 * Compares the configuration snapshot taken by {@code navigation_build_announcements()} against
 * the current values and rebuilds all announcement tables if they differ.
 *
 * @param this_ The navigation object
 */
static void navigation_announcements_check_profile(struct navigation *this_) {
    int imperial = 0, vocab_distances = 1, vocab_name = 1, vocab_systematic = 1;
    struct attr attr;
    ...
    if (this_->speech) {
        if (speech_get_attr(this_->speech, attr_vocabulary_distances, &attr, NULL))
            vocab_distances = attr.u.num;
        if (speech_get_attr(this_->speech, attr_vocabulary_name, &attr, NULL))
            vocab_name = attr.u.num;
        if (speech_get_attr(this_->speech, attr_vocabulary_name_systematic, &attr, NULL))
            vocab_systematic = attr.u.num;
    }
    if (imperial != this_->announce_imperial || vocab_distances != this_->announce_vocab_distances
        || vocab_name != this_->announce_vocab_name || vocab_systematic != this_->announce_vocab_systematic)
        navigation_build_announcements(this_);
}

/**
 * @brief Selects the speech text for a command at a given announcement level.
 *
 * This is the run-time half of announcement generation: it applies the turn-around override,
 * replicates the streetname_told bookkeeping formerly done inside {@code show_maneuver()}, and
 * returns the matching pre-rendered text from the command's announcement table. The caller
 * receives an owned copy which it must free.
 *
 * If {@code alts} is non-NULL, fallback alternatives are appended to it (owned strings): shorter
 * variants of the same announcement which are also present in the pre-rendered tables. A speech
 * plugin can use them when the primary text is not (yet) available.
 *
 * If no table has been built for the command (which should not happen), falls back to rendering
 * the text on demand.
 *
 * @param nav The navigation object
 * @param itm The navigation item starting at the vehicle's current position
 * @param cmd The command to speak an announcement for
 * @param level The announcement level to speak
 * @param alts Collects fallback alternatives, may be NULL
...
static char *navigation_get_speech_text(struct navigation *nav, struct navigation_itm *itm,
    ...
    /* Turn-around handling, formerly the prologue of show_maneuver(). Note that a connect-level
     * call returns empty without clearing the turnaround state, exactly as before. */
    ...
    if (!ann->valid)
        return show_maneuver(nav, itm, cmd, attr_navigation_speech, level, -1, cmd->itm->streetname_told);
    ...
    case level_soon:
        ...
        break;
    case level_meters:
        /* Street name bookkeeping, formerly done inside show_maneuver(): applies only to commands
         * with an actual maneuver type, mirroring the guard there. */
        if (cmd->maneuver && cmd->maneuver->type && cmd->itm->next)
            cmd->itm->streetname_told = 1; /* remember to be checked when we turn */
        src = ann->meters;
        break;
    case level_now:
        if (!cmd->itm->next) {
            src = ann->now[0]; /* destination command: never carries a street name */
        } else if (cmd->maneuver && cmd->maneuver->type && cmd->itm->streetname_told) {
    ...
    case level_connect:
        ...
        break;
    case level_follow: {
        /* The text embeds the vehicle's current distance; select the nearest pre-rendered bucket. */
        int live = round_distance(itm->dest_length - cmd->itm->dest_length);
        int best = 0, best_delta = 0, i;
        if (!ann->follow_count)
            return show_maneuver(nav, itm, cmd, attr_navigation_speech, level, -1, cmd->itm->streetname_told);
        for (i = 0; i < ann->follow_count; i++) {
            int delta = abs(ann->follow_dist[i] - live);
            if (!i || delta < best_delta) {
                best = i;
                best_delta = delta;
            }
        }
 ...
 * For speech announcements, the texts are selected from the pre-rendered announcement table of
 * the command ({@code navigation_get_speech_text()}); only decisions about whether an
 * announcement is made at all, and whether two are concatenated, are taken here at run time.
 * For route description (GUI) modes, the text is rendered on demand by {@code show_maneuver()}.
 *
 * @param nav The navigation object
 * @param itm The current navigation item, which is used to determine the distance to the next
 * maneuver. In speech mode this should be set to the navigation item starting at the vehicle's
...
static char *show_next_maneuvers(struct navigation *nav, struct navigation_itm *itm, struct navigation_command *cmd,
    ...
    if (type != attr_navigation_speech) {
        return show_maneuver(nav, itm, cmd, type, level_meters, -1,
                             0); /* We only accumulate maneuvers in speech navigation */
    }

    level = navigation_get_announce_level_cmd(nav, itm, cmd, distance - cmd->length);

    if (level > level_soon) {
        /* suppress the 'follow the road..' announcement, if the next 'soon' announcement is just a few meters away, so
         * just quit */
        if ((distance - cmd->length) < (nav->announce[itm->way.item.type - route_item_first][2]
                                        + 2 * nav->announce[itm->way.item.type - route_item_first][0])) {
        ...
        /* suppress the 'in xx meters..' announcement, if the 'soon' announcement is just a few meters away, so just
         * quit */
        if ((distance - cmd->length) < (nav->announce[itm->way.item.type - route_item_first][1]
                                        + 2 * nav->announce[itm->way.item.type - route_item_first][0])) {
            /* a few meters away is defined by two times the distance of the 'now'-distance. */
            return g_strdup("");
        }
    ...
    ret = navigation_get_speech_text(nav, itm, cmd, level, alts);
    ...
    if (cmd->next && cmd->itm) {
        ...
        enum announcement_level nextlevel = navigation_get_announce_level(
            nav, cmd->itm->way.item.type, cmd->itm->dest_length - cmd->next->itm->dest_length);

        /* If this level starts with 1 or 0 concatenate the following announcement to the current: */
        if (nextlevel <= level_soon) {
            next = navigation_get_speech_text(nav, cmd->itm, cmd->next, level_connect, alts);
...
static void navigation_call_callbacks(struct navigation *this_, int force_speech) {
    ...
    if (force_speech || this_->force_next_speech) {
        ...
        navigation_announcements_check_profile(this_);
...
static void navigation_update_done(struct navigation *this_, int cancel) {
    ...
    if (!cancel) {
        ...
        if (!(this_->status_int & status_has_sitem)) {
            ...
            profile(0, "end");
            /* Fire the status event BEFORE the maneuver callbacks: consumers use it to prepare
             * all speech output while the route is fresh, so that announcements never have to be
             * synthesized on demand when the callbacks below request them. */
            navigation_set_attr(this_, &nav_status);
...
/**
 * @brief Returns the pre-rendered speech texts for all commands.
 *
 * All texts which may ever be spoken have been generated when the maneuvers were built (see
 * {@code navigation_cmd_build_announcements()}); this function merely collects copies of them,
 * including the concatenated variants ("turn left, then turn right") for maneuvers that are close
 * to their successor. Because the strings handed out here are byte-identical to those selected at
 * speech time by {@code navigation_get_speech_text()}, a speech plugin can safely synthesize them
 * ahead of time and expect a cache hit when they are spoken later.
 *
 * @param this_ The navigation object
...
GList *navigation_prepare_all_speech(struct navigation *this_) {
    ...
    for (cmd = this_->cmd_first; cmd; cmd = cmd->next) {
        struct navigation_announcements *ann = &cmd->announcements;
        const char *base_texts[7];
        int base_levels[7];
        int i, base_count = 0;
        int concat = 0;
        char *connect_text = NULL;

        if (!ann->valid || !cmd->itm)
            continue;

        /* Only level_meters and level_now announcements can ever be concatenated with a
         * following maneuver (see show_next_maneuvers()). */
        if (ann->soon && *ann->soon) {
            base_texts[base_count] = ann->soon;
            base_levels[base_count++] = level_soon;
        }
        if (ann->meters && *ann->meters) {
            base_texts[base_count] = ann->meters;
            base_levels[base_count++] = level_meters;
        }
        for (i = 0; i < 2; i++) {
            /* Both level_now variants are collected, but identical variants are only added once. */
            if (ann->now[i] && *ann->now[i] && !(i > 0 && ann->now[i - 1] && !strcmp(ann->now[i], ann->now[i - 1]))) {
                base_texts[base_count] = ann->now[i];
                base_levels[base_count++] = level_now;
            }
        }
        for (i = 0; i < ann->follow_count; i++) {
            int j, dup = 0;
            for (j = 0; j < base_count; j++)
                if (!strcmp(base_texts[j], ann->follow[i]))
                    dup = 1;
            if (!dup) {
                base_texts[base_count] = ann->follow[i];
                base_levels[base_count++] = level_follow;
            }
        }

        if (cmd->next && cmd->next->announcements.valid && cmd->next->announcements.connect
            && *cmd->next->announcements.connect) {
            enum announcement_level nextlevel = navigation_get_announce_level(
                this_, cmd->itm->way.item.type, cmd->itm->dest_length - cmd->next->itm->dest_length);
            concat = (nextlevel <= level_soon);
            if (concat)
                connect_text = cmd->next->announcements.connect;
        }

        for (i = 0; i < base_count; i++) {
            texts = g_list_append(texts, g_strdup(base_texts[i]));
            /* Only "in X meters" and "now" announcements ever get concatenated at speech time */
            if (connect_text && base_levels[i] <= level_meters)
                texts = g_list_append(texts, g_strdup_printf("%s, %s", base_texts[i], connect_text));
        }
    }
    return texts;
}
...
void navigation_destroy(struct navigation *this_) {
    ...
    g_free(this_->speech_text);
    g_strfreev(this_->speech_alt);
    attr_list_free(this_->attrs);
    g_free(this_);
}

/**
 * @brief Returns fallback alternatives for the most recently selected speech text.
 *
 * After the {@code attr_navigation_speech} attribute has been fetched, this returns a
 * NULL-terminated array of alternative (usually shorter) texts which convey the same instruction.
 * They are pre-rendered and therefore present in the speech cache; a speech plugin can use them
 * when the primary text is not (yet) available. The returned array is owned by the navigation
 * object and valid until the next time the attribute is fetched.
 *
 * @param this_ The navigation object
 * @return A NULL-terminated array of strings, or NULL if there are no alternatives.
 */
char **navigation_get_speech_alternatives(struct navigation *this_) {
    return this_->speech_alt;
}
...
static int navigation_map_item_attr_get(void *priv_data, enum attr_type attr_type, struct attr *attr) {
    ...
    case attr_level:
        if (cmd) {
            ...
            distance = round_distance(distance);
            attr->u.num =
                navigation_get_announce_level(this_->nav, this_->cmd_itm->way.item.type, distance - cmd->length);
            return 1;
        }
        return 0;
    case attr_navigation_short:
        ...
        if (cmd) {
            this_->str = attr->u.str = show_next_maneuvers(this_->nav, this_->cmd_itm, cmd, attr_type, NULL);
            return 1;
        }
        return 0;
    ...
    case attr_name:
        ...
        if (exit_announce && maneuver)
            ...
            this_->str = attr->u.str =
                g_strdup_printf("%s %s", maneuver->merge_or_exit & mex_interchange ? (_("Interchange")) : (_("Exit")),
                                exit_announce ? exit_announce : "");
        else if (itm->way.name || itm->way.name_systematic)
            this_->str = attr->u.str = g_strdup_printf("%s %s", itm->way.name ? itm->way.name : "",
                                                       itm->way.name_systematic ? itm->way.name_systematic : "");
        if (attr->u.str) {
            return 1;
        }
        return 0;
    case attr_debug:
        ...
        case 7:
            if (this_->ways && prev) {
                this_->str = attr->u.str = g_strdup_printf(
                    "other item angle:%d delta:%d flags:%d dir:%d type:%s id:(0x%x,0x%x)", this_->ways->angle2,
                    angle_delta(prev->angle_end, this_->ways->angle2), this_->ways->flags, this_->ways->dir,
        ...
        case 8:
            ...
            if (prev) {
                ...
                this_->str = attr->u.str = g_strdup_printf(
                    "type: %s, is_complex_t_junction: %d, is_same_street: %d, is_unambiguous: %d, merge_or_exit: %d, "
                    "old_cat: %d, new_cat: %d, max_cat: %d, num_options: %d, num_similar_ways: %d, num_new_motorways: "
                    "%d, num_other_ways: %d, left: %d, right: %d, delta: %d",
                    item_to_name(maneuver->type), maneuver->is_complex_t_junction, maneuver->is_same_street,
                    maneuver->is_unambiguous, maneuver->merge_or_exit, maneuver->old_cat, maneuver->new_cat,
                    maneuver->max_cat, maneuver->num_options, maneuver->num_similar_ways, maneuver->num_new_motorways,
                    maneuver->num_other_ways, maneuver->left, maneuver->right, maneuver->delta);
                return 1;
            }
            /* fall through */
...
static struct item *navigation_map_get_item(struct map_rect_priv *priv) {
    ...
    if (priv->cmd->itm == priv->itm) {
        ...
        if (priv->cmd_itm_next && !priv->cmd_itm_next->next)
            ret->type = type_nav_destination; /* FIXME: do we need to set that here? The generic case should catch that
                                                 now... */
        else if (priv->cmd->maneuver
                 && ((priv->cmd->maneuver->type != type_nav_none)
