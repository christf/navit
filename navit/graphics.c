...
void graphics_free(struct graphics *gra) {
    ...
    if (!gra->parent) {
        ...
        /* We can't specify context (pointer to struct graphics) for g_hash_table_new to have it passed to free function
           so we have to free img->priv manually, the rest would be freed by g_hash_table_destroy. GHashTableIter isn't
           used because it broke n800 build at r5107.
        */
...
static void image_new_helper(struct graphics *gra, struct graphics_image *this_, char *path, char *name, int width,
    ...
    while (mode <= 8) {
        ...
        case 7:
            ...
            break;
        case 8:
            /* xpm format is used as a last resort, because its not widely supported and we are moving to svg and png
             * formats */
            mode++;
        ...
        this_->priv =
            gra->meth.image_new(gra->priv, &this_->meth, new_name, &this_->width, &this_->height, &this_->hot, rotate);
...
struct graphics_image *graphics_image_new_scaled_rotated(struct graphics *gra, char *path, int w, int h, int rotate) {
    ...
    for (i = 0; i < file_wordexp_get_count(we) && !this_->priv; i++) {
        ...
        /* If exact h and w values were given as function parameters, they take precedence over values guessed from the
         * image name */
...
void graphics_draw_rectangle(struct graphics *this_, struct graphics_gc *gc, struct point *p, int w, int h) {
    struct point p_scaled;
    p_scaled = graphics_dpi_scale_point(this_, p);
    this_->meth.draw_rectangle(this_->priv, gc->priv, &p_scaled, graphics_dpi_scale(this_, w),
                               graphics_dpi_scale(this_, h));
}
...
#pragma GCC diagnostic ignored "-Wcast-align"
    ...
    *p += hole_count * sizeof(struct coord *);
    ...
    while (item_attr_get(item, attr_poly_hole, &attr)) {
        ...
        memcpy(holes->coords[holes->count], attr.u.poly_hole->coord,
               holes->ccount[holes->count] * sizeof(struct coord));
...
static void display_add(struct hash_entry *entry, struct item *item, int count, struct coord *c, char **label,
    ...
    holes_length = sizeof(struct displayitem_poly_holes) + hole_count * sizeof(int)
                   + hole_count * sizeof(struct coord *) + hole_total_coords * sizeof(struct coord);
    if (hole_count > 0)
...
/* A label placed in the current frame, used to suppress duplicate labels. x/y
 * is the anchor, text_size the font size, xmin..ymax the rotated bounding box. */
struct label_pos {
    int x, y;
...
/* Reset the per-frame registry of placed labels. Called at the start of each
 * frame (graphics_draw_mode) and from graphics_free(). Drawing is
...
/* Compute the axis-aligned bounding box of a rotated label drawn from its
 * anchor along the direction (dirx, diry). The four text corners are
 * anchor + along * {0, tl} + perpendicular * {+-th/2}. Fills in the anchor and
 * the bounding box; text_size is set by the caller. */
...
/* Fallback character cell size in screen units, used when no text bbox lookup is available. */
#define LABEL_CHAR_W 4
#define LABEL_CHAR_H 8

/* Compute the on-screen width of a single line of text. */
...
/* Same label footprint as label_is_duplicate() uses, so culling and dedup
 * agree on what a label covers. */
...
/* The bounding box of a label is inflated by this many extra line heights on
 * every side before checking for overlap, so labels that merely touch or sit
 * close to each other are still treated as a collision. */
#define LABEL_GUTTER_HEIGHTS 1

/* Check whether a label duplicates one already placed in this frame. Only the
 * first of several identical labels that are close together on screen is
 * drawn, so e.g. a street name also carried by its mapped sidewalks, cycleways
 * and footways is only rendered once. A label is a duplicate if its bounding
 * box, inflated by a spacing gutter, overlaps one already placed (this also
 * catches labels on perpendicular arms of a street that cross at a corner), or
 * if its anchor is within half a label width (at least two line heights) of
 * one already placed. Repetition of a name along a long street is unaffected,
 * since those anchors are at least a full label width apart. A label with a
 * larger text size is never suppressed by a smaller duplicate; label_record()
 * replaces the smaller duplicates a larger label covers. Returns 1 if the
 * label should be skipped, 0 if it should be drawn. */
static int label_is_duplicate(struct graphics *gra, const char *label, int x, int y, int tl, int th, int text_size,
    ...
    bbox.xmin -= LABEL_GUTTER_HEIGHTS * th;
    bbox.ymin -= LABEL_GUTTER_HEIGHTS * th;
    bbox.xmax += LABEL_GUTTER_HEIGHTS * th;
    bbox.ymax += LABEL_GUTTER_HEIGHTS * th;
    for (; l; l = l->next) {
...
/* Record a label that has been placed, so later duplicates can be suppressed.
 * A larger label replaces smaller duplicates of the same text that it covers,
 * so the registry keeps the largest placement for a text. */
static void label_record(struct graphics *gra, const char *label, struct label_pos pos) {
    ...
    *lp = pos;
    ...
    old = g_hash_table_lookup(gra->placed_labels, label);
    for (n = old; n; n = n->next) {
        struct label_pos *p = n->data;
        if (!(p->text_size < pos.text_size && labels_intersect(p, lp))) {
            struct label_pos *keep = g_new(struct label_pos, 1);
            *keep = *p;
            l = g_list_prepend(l, keep);
        }
    }
    /* insert() frees the old key-value pair, dropping the smaller duplicates */
    g_hash_table_insert(gra->placed_labels, g_strdup(label), g_list_prepend(l, lp));
}
...
static void display_draw_arrow(struct point *p, navit_float dx, navit_float dy, navit_float width,
                               struct display_context *dc, struct graphics *gra, int filled) {
    struct point pnt[4];
...
static void display_draw_spike(struct point *p, navit_float dx, navit_float dy, navit_float width,
                               struct display_context *dc, struct graphics *gra) {
    struct point pnt[2];
...
static int intersection(struct point *a1, int adx, int ady, struct point *b1, int bdx, int bdy, struct point *res) {
    int n, a, b;
    dbg(lvl_debug, "%d,%d - %d,%d x %d,%d-%d,%d", a1->x, a1->y, a1->x + adx, a1->y + ady, b1->x, b1->y, b1->x + bdx,
        b1->y + bdy);
    n = bdy * adx - bdx * ady;
 ...
 * @param scale Unused
 * @param start Position of the first point on the circle (in 1/1024th of the circle), -1 being the bottom of the
 * circle, 511 being the top of the circle
 * @param len Length of the arc on the circle, relative to start (in 1/1024th of the circle), 514 is half a circle, 1028
 * is a full circle (or 1027 if first and last points are connected with a line)
 ...
 * @param dir Direction of the circle (valid values are 1 (counter-clockwise) or -1 (clockwise), other values may lead
 * to unknown result)
 */
static void circle_to_points(const struct point *center, int diameter, int scale, int start, int len, struct point *res,
                             int *pos, int dir) {
...
static void graphics_draw_polyline_as_polygon(struct graphics *gra, struct graphics_gc *gc, struct point *pnt,
                                              int count, int *width) {
    int maxpoints = 200;
 ...
 * @param[in] pin An array of points forming the polygon
 * @param count_in The number of elements inside @p pin
 * @param[in] width An array of width matching the line starting from the corresponding @p pa (if all equal, all lines
 * will have the same width)
 * @param poly A boolean indicating whether the polyline should be closed to form a polygon (only the contour of this
...
void graphics_draw_polyline_clipped(struct graphics *gra, struct graphics_gc *gc, struct point *pa, int count,
    ...
    for (i = 0; i < count; i++) {
        if (i) {
            ...
            dbg(lvl_debug, "Segment: [%d, %d] - [%d, %d]...", segment_start.x, segment_start.y, segment_end.x,
                segment_end.y);
            clip_result = clip_line(&segment_start, &segment_end, &r);
            if (clip_result != CLIPRES_INVISIBLE) {
                dbg(lvl_debug, "....clipped to [%d, %d] - [%d, %d]", segment_start.x, segment_start.y, segment_end.x,
                    segment_end.y);
...
static inline void displayitem_transform_holes(struct transformation *trans, enum projection pro,
                                               struct displayitem_poly_holes *in, struct displayitem_poly_holes *out,
                                               int mindist) {
    if (out == NULL)
        return;
    out->count = 0;
    ...
    if ((in != NULL) && (in->count > 0)) {
        ...
        for (a = 0; a < in->count; a++) {
            ...
            while (transform_res == TRANSFORM_ERR_BUF_SPACE) {
                ...
                transform_res = transform_point_buf(trans, pro, in->coords[a], (struct point *)(out->coords[a]),
                                                    buf_size, in->ccount[a], mindist, 0, NULL);
            }
 ...
 * Use this function to get map selection if you are going to fetch complete item data from the map based on displayitem
 * reference.
 * @param displaylist
...
/**
 * @brief Returns list of displayitems clicked at given coordinates. The deeper item is in current layout, the deeper it
 * will be in the list.
 * @param displaylist
        ...
        if (displaylist->mr) {
            while ((item = map_rect_get_item(displaylist->mr))) {
                ...
                if (count == max) {
                    ...
                    dbg(lvl_error, "point count overflow %d for %s " ITEM_ID_FMT ". Increase to %d", count,
                        item_to_name(item->type), ITEM_ID_ARGS(*item), displaylist->dc.maxlen);
                    ...
                    item_coord_rewind(item);
                    count =
                        item_coord_get_within_selection(item, ca, item->type < type_line ? 1 : max, displaylist->sel);
 ...
 * NOTE: returned structure doesn't contain any attributes or coordinates. type, map, idhi and idlow seem to be the only
 * useable members.
...
/**
 * Get the number of this item as it was last displayed on the screen, dependent of current layout. Items with lower
 * numbers are shaded by items with higher ones when they overlap. Zero means item was not displayed at all. If the item
 * is displayed twice, its topmost occurence is used.
 * @param di pointer to displayitem structure
...
static int within_dist_polygon(struct point *p, struct point *poly_pnt, int count, int dist) {
    ...
    for (i = 0, j = count - 1; i < count; j = i++) {
        if ((((poly_pnt[i].y <= p->y) && (p->y < poly_pnt[j].y)) || ((poly_pnt[j].y <= p->y) && (p->y < poly_pnt[i].y)))
            && (p->x < (poly_pnt[j].x - poly_pnt[i].x) * (p->y - poly_pnt[i].y) / (poly_pnt[j].y - poly_pnt[i].y)
                           + poly_pnt[i].x))
            c = !c;
    }
    if (!c)
...
int graphics_displayitem_within_dist(struct displaylist *displaylist, struct displayitem *di, struct point *p,
    ...
    count =
        transform_point_buf(displaylist->dc.trans, displaylist->dc.pro, di->c, pa, pa_buf_size, di->count, 0, 0, NULL);
