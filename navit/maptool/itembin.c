...
void item_bin_write_clipped(struct item_bin *ib, struct tile_parameter *param, struct item_bin_sink *out) {
    ...
    tile_data.tile_depth =
        tile(&tile_data.item_bbox, NULL, tile_data.buffer, param->max, param->overlap, &tile_data.tile_bbox);
...
void clip_line(struct item_bin *ib, struct rect *r, struct tile_parameter *param, struct item_bin_sink *out) {
    ...
    for (i = 0; i < count; i++) {
        if (i) {
            ...
            p2.x = pa[i].x;
            p2.y = pa[i].y;
            /* 0 = invisible, 1 = completely visible, 3 = start point clipped, 5 = end point clipped, 7 both points
             * clipped */
