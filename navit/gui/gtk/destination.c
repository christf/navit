...
static void button_map(GtkWidget *widget, struct search_param *search) {
    ...
    if (point) {
        ...
        result->c.y = point->y;
        result->label =
            description(search, &iter); /* Allocated here, freed by invoking g_free() below when parsing the GList */
...
static void set_columns(struct search_param *param, int mode) {
    ...
    while (*column_text) {
        ...
        gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(param->treeview), -1, navit_nls_gettext(*column_text),
                                                    cell, "text", i, NULL);
        i++;
        column_text++;
    }
...
int destination_address(struct navit *nav) {
    ...
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(search->listbox), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
