...
static void statusbar_route_update(struct statusbar_priv *this, struct navit *navit, struct vehicle *v) {
    ...
    sprintf(buffer, _("Route %4.1f%s    %02d:%02d ETA"),
            ...
            imperial == TRUE ? "mi" : "km", eta_tm ? eta_tm->tm_hour : 0, eta_tm ? eta_tm->tm_min : 0);
    ...
    snprintf(this->gps_text, sizeof(this->gps_text), "GPS:%s %02d/%02d HD:%02.2f %s %4.0f%s %3.0f°%-2s %3.1f%s",
             status_fix2str(status), sats, qual, hdop, buffer, imperial ? height * FEET_PER_METER : height,
             imperial == TRUE ? "\'" : "m", direction, dir, imperial == TRUE ? speed * KILOMETERS_TO_MILES : speed,
