...
static void ch_generate_sgr(char *suffix) {
    ...
    int system_result;
    sprintf(command,
            "./contraction-hierarchies-20080621/main -s -p -f ddsg_%s.tmp -o hcn_%s.tmp -l hcn_log_%s.tmp -x 190 -y 1 "
            "-e 600 -p 1000 -k 1,3.3,2,10,3,10,5",
    ...
    sprintf(command,
            "./contraction-hierarchies-20080621/main -c -f ddsg_%s.tmp -h hcn_%s.tmp -k 1,3.3,2,10,3,10,5 -C ch_%s.tmp "
            "-O 1 -z sgr_%s.tmp",
