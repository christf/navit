...
- the ``textfile`` map format
- the ``center`` attribute in the configuration file
- some Navit commands (e.g. ``set_position``), which can be invoked via the internal GUI or the D-Bus bindings
...
Latitude / Longitude can also be specified in degrees and minutes with compass directions (N/S, E/W):
...
Navit can read coordinates in the `Universal Transverse Mercator coordinate system <http://en.wikipedia.org/wiki/Universal_Transverse_Mercator_coordinate_system>`_ (UTM).
