...
typedef BOOL(WINAPI *FP_AlphaBlend)(HDC hdcDest, int nXOriginDest, int nYOriginDest, int nWidthDest, int nHeightDest,
...
static void create_memory_dc(struct graphics_priv *gr) {
    ...
    gr->hPrebuildBitmap =
        CreateDIBSection(gr->hMemDC, &bOverlayInfo, DIB_RGB_COLORS, (void **)&gr->pPixelData, NULL, 0);
    ...
    case WM_PAINT:
        if (gra_priv && gra_priv->hMemDC) {
            ...
            while (!gra_priv->disabled && overlay) {
                if (!overlay->disabled && overlay->p.x >= 0 && overlay->p.y >= 0 && overlay->p.x < gra_priv->width
                    ...
                    if (!overlay->hPrebuildDC) {
                        ...
                        SetBkColor(overlay->hMemDC,
                                   RGB(overlay->transparent_color.r >> 8, overlay->transparent_color.g >> 8,
                                       overlay->transparent_color.b >> 8));
                        BitBlt(overlay->hPrebuildDC, 0, 0, overlay->width, overlay->height, overlay->hMemDC, 0, 0,
                               SRCCOPY);
                        BitBlt(overlay->hMemDC, 0, 0, overlay->width, overlay->height, overlay->hPrebuildDC, 0, 0,
                               SRCINVERT);
                    }

#else
                    const COLORREF transparent_color = RGB(overlay->transparent_color.r >> 8,
                                                           overlay->transparent_color.g >> 8,
                                                           overlay->transparent_color.b >> 8);
                    ...
                    for (y = 0; y < h; y++) {
                        for (x = 0; x < w; x++) {
                            ...
                            if (overlay->pPixelData[srcPixel] == transparent_color) {
                                ...
                                gra_priv->pPixelData[destPixel] = RGB(
                                    ((65535 - overlay->transparent_color.a) * GetRValue(gra_priv->pPixelData[destPixel])
                                     + overlay->transparent_color.a * GetRValue(overlay->pPixelData[srcPixel]))
                                        / 65535,
                                    ((65535 - overlay->transparent_color.a) * GetGValue(gra_priv->pPixelData[destPixel])
                                     + overlay->transparent_color.a * GetGValue(overlay->pPixelData[srcPixel]))
                                        / 65535,
                                    ((65535 - overlay->transparent_color.a) * GetBValue(gra_priv->pPixelData[destPixel])
                                     + overlay->transparent_color.a * GetBValue(overlay->pPixelData[srcPixel]))
                                        / 65535);
            ...
            while (!gra_priv->disabled && overlay && !overlay->disabled) {
                if (overlay->p.x > 0 && overlay->p.y > 0 && overlay->p.x + overlay->width < gra_priv->width
                    && overlay->p.y + overlay->height < gra_priv->height) {
                    BitBlt(hdc, overlay->p.x, overlay->p.y, overlay->width, overlay->height, overlay->hPrebuildDC, 0, 0,
                           SRCAND);
                    BitBlt(hdc, overlay->p.x, overlay->p.y, overlay->width, overlay->height, overlay->hMemDC, 0, 0,
                           SRCPAINT);
                }
                overlay = overlay->next;
            }
...
#    endif
...
#ifdef HAVE_API_WIN32_CE
    g_hwnd = hwnd = CreateWindow(g_szClassName, TEXT("Navit"), wStyle, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                                 CW_USEDEFAULT, gr->wnd_parent_handle, hMenu, GetModuleHandle(NULL), NULL);
#else
    g_hwnd = hwnd = CreateWindow(g_szClassName, TEXT("Navit"), wStyle, gr->x, gr->y, gr->width, gr->height,
    ...
    /* For Vista, we need here ChangeWindowMessageFilter(WM_COPYDATA,MSGFLT_ADD); since Win7 we need above one or
      ChangeWindowMessageFilterEx (MSDN), both are not avail for earlier Win and not present in my mingw :(.
      ChangeWindowMessageFilter may not be present in later Win versions. Welcome late binding!
    */
...

    {
        ...
        if (convertResult > 0) {  // Convert was ok
            if (bg && bg->fg_alpha) {
                ...
                ExtTextOutW(gr->hMemDC, -1, -1, 0, NULL, utf16, convertResult - 1, NULL);
                ExtTextOutW(gr->hMemDC, 1, 1, 0, NULL, utf16, convertResult - 1, NULL);
                ExtTextOutW(gr->hMemDC, -1, 1, 0, NULL, utf16, convertResult - 1, NULL);
                ExtTextOutW(gr->hMemDC, 1, -1, 0, NULL, utf16, convertResult - 1, NULL);
            }
            SetTextColor(gr->hMemDC, fg->fg_color);
            ExtTextOutW(gr->hMemDC, 0, 0, 0, NULL, utf16, convertResult - 1, NULL);
        }
    }
...
static struct graphics_image_priv *image_new(struct graphics_priv *gr, struct graphics_image_methods *meth, char *name,
    ...
    if (!g_hash_table_lookup_extended(gr->image_cache_hash, hash_key, NULL, (gpointer)&ret)) {
        ...
        /* Hash_key will be freed ater the hash table, so set it to NULL here to disable freing it on this function
         * return */
        hash_key = NULL;
        if (ret) {
...
#else
        gra_priv->SetStretchBltMode =
            (FP_SetStretchBltMode)GetProcAddress(gra_priv->hGdi32Dll, TEXT("SetStretchBltMode"));
#endif
...
static struct event_methods event_win32_methods = {
    .main_loop_run = event_win32_main_loop_run,
    .main_loop_quit = event_win32_main_loop_quit,
    .add_watch = event_win32_add_watch,
    .remove_watch = event_win32_remove_watch,
    .add_timeout = event_win32_add_timeout,
    .remove_timeout = event_win32_remove_timeout,
    .add_idle = event_win32_add_idle,
    .remove_idle = event_win32_remove_idle,
    .call_callback = event_win32_call_callback,
};
