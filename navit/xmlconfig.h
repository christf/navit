...
struct object_func {
    ...
    int (*get_attr)(void *, enum attr_type type, struct attr *attr,
                    struct attr_iter *iter);       /**< Function
...
extern struct object_func map_func, mapset_func, navit_func, osd_func, tracking_func, vehicle_func, maps_func,
    layout_func, roadprofile_func, vehicleprofile_func, layer_func, config_func, profile_option_func, script_func,
    log_func, speech_func, synthesizer_func, navigation_func, route_func, traffic_func;

#define HAS_OBJECT_FUNC(x)                                                                                             \
    ((x) == attr_map || (x) == attr_mapset || (x) == attr_navit || (x) == attr_osd || (x) == attr_trackingo            \
     || (x) == attr_vehicle || (x) == attr_maps || (x) == attr_layout || (x) == attr_roadprofile                       \
     || (x) == attr_vehicleprofile || (x) == attr_layer || (x) == attr_config || (x) == attr_profile_option            \
     || (x) == attr_script || (x) == attr_log || (x) == attr_speech || (x) == attr_synthesizer                         \
     || (x) == attr_navigation || (x) == attr_route || (x) == attr_traffic)
...
// static void xinclude(GMarkupParseContext *context, const gchar **attribute_names, const gchar **attribute_values,
