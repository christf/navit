...
#define DEGREES_FULL_TURN 360
#define DEGREES_HALF_TURN 180
...
struct vehicle *vehicle_new(struct attr *parent, struct attr **attrs) {
    ...
    struct attr *source;
    struct vehicle_priv *(*vehicletype_new)(struct vehicle_methods *meth, struct callback_list *cbl,
                                            struct attr **attrs);
    char *type, *colon;
 ...
 * @param iter A vehicle attr_iter. This is only used for generic attributes; for attributes specific to the vehicle
...
void vehicle_set_cursor(struct vehicle *this_, struct cursor *cursor, int overwrite) {
    dbg(lvl_debug, "enter this_=%p cursot=%p overwrit=%d, this_->cursor_fixed=%d, this_->gra=%p", this_, cursor,
        overwrite, this_->cursor_fixed, this_->gra);
...
void vehicle_interpolate(struct vehicle *this_) {
    ...
    elapsed = (now.tv_sec - this_->interp_t0.tv_sec) * MS_PER_SEC
              + (now.tv_usec - this_->interp_t0.tv_usec) / MS_PER_SEC;
    ...
    {
        int delta = ((this_->interp_target_yaw - this_->interp_prev_yaw + DEGREES_HALF_TURN) % DEGREES_FULL_TURN
                     + DEGREES_FULL_TURN)
                        % DEGREES_FULL_TURN
                    - DEGREES_HALF_TURN;
        int yaw = this_->interp_prev_yaw + (int)(t * delta);
        yaw = yaw % DEGREES_FULL_TURN;
        if (yaw < 0)
            yaw += DEGREES_FULL_TURN;
