 ...
 * @param move_only If non nul, transfer only the second widget into the first widget. The second widget is then
 * deallocated and should be be used anymore (this is a move operation)
 */
static void gui_internal_widget_transfer_content(struct gui_priv *this, struct widget *first, struct widget *second,
    ...
    if (move_only) {
        ...
        /* Now, we also free the struct widget pointed to by second, so variable second should not be used anymore from
         * this point */
        g_free(second);
    } else {
...
/**
 * @brief Move the content of one widget into another one (the @p src widget is then deallocated and should be be used
 * anymore)
 *
 * @param this The internal GUI instance
...
static void gui_internal_label_render(struct gui_priv *this, struct widget *w) {
    ...
    if (w->text) {
        ...
        if (w->flags & gravity_right) {
            ...
            graphics_draw_text(this->gra, w->foreground, w->text_background, this->fonts[w->font_idx], text, &pnt,
                               0x10000, 0x0);
        } else {
            pnt.y += w->h - this->spacing;
            graphics_draw_text(this->gra, w->foreground, w->text_background, this->fonts[w->font_idx], text, &pnt,
    ...
    if (ret) {
        ...
        if (text)
            gui_internal_widget_append(
                ret, gui_internal_text_font_new(this, text, font, gravity_center | orientation_vertical));
        if (image && (flags & flags_swap))
            gui_internal_widget_append(ret, gui_internal_image_new(this, image));
        ret->func = func;
        ret->data = data;
...
void gui_internal_box_pack(struct gui_priv *this, struct widget *w) {
    struct widget *wc;
    int x0, x = 0, y = 0, width = 0, height = 0, owidth = 0, oheight = 0, expand = 0, expandd = 1, count = 0, rows = 0,
            cols = w->cols ? w->cols : 0;
...
void gui_internal_box_resize(struct gui_priv *this, struct widget *w, void *data, int wnew, int hnew) {
    ...
    /* Note: this widget and its children have been resized, a call to gui_internal_box_render() needs to be done by the
     * caller */
}
...
static void gui_internal_scroll_buttons_init(struct gui_priv *this, struct widget *widget, struct scroll_buttons *sb) {
    sb->next_button = gui_internal_button_new_with_callback(this, _("Next"), image_new_xs(this, "gui_arrow_right"),
                                                            gravity_center | orientation_horizontal | flags_swap,
                                                            gui_internal_table_button_next, widget);
    sb->prev_button = gui_internal_button_new_with_callback(this, _("Prev"), image_new_xs(this, "gui_arrow_left"),
                                                            ...
                                                            gui_internal_table_button_prev, widget);
...
static GList *gui_internal_compute_table_dimensions(struct gui_priv *this, struct widget *w) {
    ...
    for (cur_row = w->children; cur_row; cur_row = g_list_next(cur_row)) {
        ...
        column_count = 0;
        for (cur_column = cur_row_widget->children; cur_column; cur_column = g_list_next(cur_column)) {
        ...
        for (cur_column = cur_row_widget->children; cur_column; cur_column = g_list_next(cur_column)) {
            ...
            if (drawing_space_left) {
                ...
                if (visual_debug) {
                    ...
                    pnt[4] = pnt[0];
                    graphics_draw_lines(this->gra, debug_gc, pnt,
                                        5); /* Force highlighting table borders in debug more */
                }
#endif
