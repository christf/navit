...
static dbg_level parse_dbg_level(struct attr *dbg_level_attr, struct attr *level_attr) {
    if (dbg_level_attr) {
        ...
        if (level_attr->u.num >= lvl_error && level_attr->u.num <= lvl_debug)
 ...
 *
 * Timestamp has the format "HH:MM:SS:mmm|" (with mmm=milliseconds), or under Windows "SSSSS:uuuuuu|" (with
 * uuuuuu=microseconds)
 *
...
static void debug_timestamp(char *buffer) {
    ...
    if (gettimeofday(&tv, NULL) == -1)
        return;
    /* Timestamps are UTC */
    sprintf(buffer, "%02d:%02d:%02d.%03d|", (int)(tv.tv_sec / 3600) % 24, (int)(tv.tv_sec / 60) % 60,
 ...
 *
 * @param level The level of the message. The message will only be written if \p level is higher than the minimum
 * (global, per module or per function)
 ...
 * @param prefix Force prepending the message with context information (a timestamp, if timestamp_prefix is set), and
...
void debug_vprintf(dbg_level level, const char *module, const int mlen, const char *function, const int flen,
     ...
     * "gui_internal:gui_internal_set_attr\0" with '\0' being the last byte (stored in
    ...
    *message_origin = ':';
    ...
    /* The source of the debug message has been created, is terminated with '\0' and stored at the very end of the
    ...
    if (global_debug_level >= level || debug_level_get(module) >= level || debug_level_get(message_origin) >= level) {
        ...
        if (prefix) {
            ...
            /* When we reach this part of the code, end is a pointer to the beginning of the debug message (inside the
             * buffer debug_message) */
            g_strlcpy(end, dbg_level_to_string(level),
                      sizeof(debug_message)
                          - (end - debug_message)); /* Add the debug level for the current debug message level */
            len = strlen(debug_message);
            ...
            *end++ = ':';
            ...
            memmove(end, message_origin,
                    len);          /* We use memmove here as both message_origin and destination may overlap */
            message_origin = NULL; /* Warning: from this point, we must not use the pointer message_origin as the
                                      content of the debug_message may be overwritten at any time. */
            end += len;
            dbg_assert(end + 1 < debug_message + sizeof(debug_message)); /* Make sure we don't get any overflow for both
                                                                            ':' and terminating '\0' */
            *end++ = ':';
...
#    define vsnprintf _vsnprintf
        ...
        if (len + 1 < sizeof(debug_message) - 1) {
            /* For Windows platforms, add \r at the end of the buffer (if any room), make sure that we have room for one
             * more character */
            *end++ = '\r';
            len++;
            *end = '\0';
