...
#include "callback.h"
/* Default render margin is one quarter of the larger screen dimension. */
#define RENDER_MARGIN_DEFAULT_DIVISOR 4
#include "color.h"
#include "config.h"
#include "debug.h"
#include "event.h"
...
static void draw_func(GtkDrawingArea *area, cairo_t *cr, int width, int height, gpointer user_data) {
    ...
    if (gra->width != width || gra->height != height) {
        ...
        gra->width = width;
        gra->height = height;
        gra->render_margin = (width > height ? width : height) / RENDER_MARGIN_DEFAULT_DIVISOR;
