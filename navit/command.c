...
static int command_object_get_attr(struct context *ctx, struct attr *object, enum attr_type attr_type,
    ...
    if (!object->u.data || !func || !func->get_attr) {
        dbg(lvl_warning, "cannot retrieve attributes from %s (%p), func=%p", attr_to_name(object->type), object->u.data,
            func) return 0;
    }
    ...
    if (r)
        ...
        dbg(lvl_warning, "%s (%p) has no attribute %s", attr_to_name(object->type), object->u.data,
            attr_to_name(attr_type)) return r;
}
    ...
    while (*ctx->expr) {
        ...
        if ((res->attr.type == attr_none) && (res->varlen > 0)) {
            if (ctx->expr[0] != '.') {
                ...
                return 2;  // 2 means "there are more object names following" (e.g. we just hit 'vehicle' in
    ...
    if (!ctx->skip) {
        if (!strcmp(function, "_") && list && list[0] && list[0]->type >= attr_type_string_begin
            ...
            if (command_object_get_attr(ctx, &res->attr, attr_callback_list, &cbl)) {
                ...
                if (valid != 1) {
                    dbg(lvl_error,
                        "invalid command ignored: \"%s\"; see "
                        "https://navit.readthedocs.io/en/latest/user/configuration/OSD.html#navit-commands"
                        " for valid commands.",
                        function);
                }
                if (out && out[0]) {
        ...
        if (cs->prev.type != attr_none) {
            ...
            if (func->add_attr) {
                if (status == 2) {  // This is not the final attribute name
                    cb = callback_new_attr_1(callback_cast(command_saved_callbacks_changed), cs->res.attr.type,
                                             (void *)cs);
