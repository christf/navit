...
/**
 * @brief Transform the coordinates of a geographical point from a coord representation to a geographical (lat, long)
 * representation
 *
...
/**
 * @brief Transform the coordinates of a geographical point from a geographical (lat, long) representation to a coord
 * representation
 *
...
static int transform_points_too_close(struct point screen_point, struct point screen_point_old, int mindist) {
    ...
    return (abs(screen_point.x - screen_point_old.x) + abs(screen_point.y - screen_point_old.y)) < mindist;
}
...
static struct z_clip_result transform_z_clip_if_necessary(struct coord_3d coord, int zlimit,
    ...
    struct z_clip_result clip_result = {
        {0, 0},
        0, 0, 0
    };
...
int transform_point_buf(struct transformation *t, enum projection required_projection, struct coord *input,
    ...
    int zlimit = t->znear;
    struct z_clip_result clip_result, clip_result_old = {
                                          {0, 0},
                                          -1, 0, 0
    };
    ...
    for (i = 0; i < count; i++) {
        ...
        if (i != 0 && i != count - 1 && (input[i + 1].x != input[0].x || input[i + 1].y != input[0].y)) {
...
static int transform_reverse_near_far(struct transformation *t, struct point *p, struct coord *c, int near_dist,
                                      int far_dist) {
    double xc, yc;
    dbg(lvl_debug, "%d,%d", p->x, p->y);
    if (t->ddd) {
        struct coord_geo_cart nearc, farc, nears, fars, intersection;
        transform_screen_to_3d(t, p, near_dist, &nearc);
        transform_screen_to_3d(t, p, far_dist, &farc);
        ...
        *msm = *ms;
        ...
        if (t->ddd) {
            ...
            for (i = 0; i < 4; i++) {
                ...
                dbg(lvl_debug, "map(%d) %d,%d=0x%x,0x%x", i, screen_pnt[i].x, screen_pnt[i].y, screen[i].x,
                    screen[i].y);
            }
        ...
        dbg(lvl_debug, "%dx%d", msm->u.c_rect.rl.x - msm->u.c_rect.lu.x, msm->u.c_rect.lu.y - msm->u.c_rect.rl.y);
        *msm_last = msm;
        msm_last = &msm->next;
        ms = ms->next;
    }
...
#ifdef AVOID_FLOAT
static int tab_sqrt[] = {14142, 13379, 12806, 12364, 12018, 11741, 11517, 11333, 11180, 11051, 10943,
                         10850, 10770, 10701, 10640, 10587, 10540, 10499, 10462, 10429, 10400, 10373,
                         10349, 10327, 10307, 10289, 10273, 10257, 10243, 10231, 10219, 10208};

static int tab_int_step = 0x20000;
static int tab_int_scale[] = {10000, 10002, 10008, 10019, 10033, 10052, 10076, 10103, 10135, 10171, 10212, 10257,
                              10306, 10359, 10417, 10479, 10546, 10617, 10693, 10773, 10858, 10947, 11041, 11140,
                              11243, 11352, 11465, 11582, 11705, 11833, 11965, 12103, 12246, 12394, 12547, 12706,
                              12870, 13039, 13214, 13395, 13581, 13773, 13971, 14174, 14384, 14600, 14822, 15050,
                              15285, 15526, 15774, 16028, 16289, 16557, 16832, 17114, 17404, 17700, 18005, 18316,
                              18636, 18964, 19299, 19643, 19995, 20355, 20724, 21102, 21489, 21885, 22290, 22705,
                              23129, 23563, 24007, 24461, 24926, 25401, 25886, 26383, 26891};
...
#ifdef AVOID_FLOAT
static int tab_atan[] = {0,    262,  524,  787,  1051, 1317, 1584, 1853, 2126,  2401, 2679,
                         2962, 3249, 3541, 3839, 4142, 4452, 4770, 5095, 5430,  5774, 6128,
                         6494, 6873, 7265, 7673, 8098, 8541, 9004, 9490, 10000, 10538};
...
int transform_within_border(struct transformation *this_, struct point *p, int border) {
    ...
    while (ms) {
        ...
        if (p->x >= r->lu.x + border && p->x <= r->rl.x - border && p->y >= r->lu.y + border
            && p->y <= r->rl.y - border)
            return 1;
        ms = ms->next;
    }
...
int transform_within_dist_polygon(struct coord *ref, struct coord *c, int count, int dist) {
    ...
    for (i = 0, j = count - 1; i < count; j = i++) {
        if ((((c[i].y <= ref->y) && (ref->y < c[j].y)) || ((c[j].y <= ref->y) && (ref->y < c[i].y)))
...
/*
Note: there are many mathematically equivalent ways to express these formulas. As usual, not all of them are
