...
static char *town_display_label(struct search_list_result *res, int level, int flags) {
    ...
    return g_strdup_printf("%s%s%s%s%s%s%s%s", postal, postal_sep, town, district_begin, district, district_end,
                           county_sep, county);
}
        ...
        while (lk) {
            ...
            while (lk2) {
                ...
                if (child_->data && strcmp("\b", child_->data) && child_->data_free) {
                    if ((strlen(possible_keys) == 0) || (g_strrstr(possible_keys, child_->data) != NULL)) {
                        if (this->hide_keys) {
...
static struct widget *gui_internal_create_resultlist_entry(struct gui_priv *this, struct search_list_result *res,
                                                           char *result_main_label, char *result_sublabel, void *param,
                                                           char *widget_name, struct item *item) {
    ...
    if (result_sublabel) {
        ...
        resultlist_entry = gui_internal_button_new_with_callback(
            this, result_main_label, image_new_xs(this, res->country->flag),
            gravity_left_center | orientation_horizontal | flags_fill, gui_internal_cmd_position, param);
    }
...
static void gui_internal_search_idle(struct gui_priv *this, char *wm_name, struct widget *search_list, void *param) {
    ...
    resultlist_entry =
        gui_internal_create_resultlist_entry(this, res, result_main_label, result_sublabel, param, widget_name, item);
...
void gui_internal_search(struct gui_priv *this, const char *what, const char *type, int flags) {
    ...
    keyboard_mode = VKBD_FLAG_2
                    | gui_internal_keyboard_init_mode(this->country_iso2 ? this->country_iso2 : getenv("LANG"));
    ...
    wl = gui_internal_widget_table_new(
        this, gravity_left_top | flags_fill | flags_expand | orientation_vertical,
