            ...
            if (transform_reverse(this_->trans, &effective, &cursor_map)) {
                ...
                }
                {
                    ...
                    if (this_->vehicle && this_->vehicle->vehicle
                        && vehicle_get_attr(this_->vehicle->vehicle, attr_position_coord_geo, &raw_attr, NULL)) {
                        struct coord raw;
                        transform_from_geo(pro, raw_attr.u.coord_geo, &raw);
                        snap_dist = transform_distance(pro, &raw, &closest);
                        dbg(lvl_debug,
                            "road_gap: cursor_map=(%d,%d) closest=(%d,%d) raw=(%d,%d) "
                            "road_dist=%.1fm snap_dist=%.1fm drag=(%d,%d)",
                            cursor_map.x, cursor_map.y, closest.x, closest.y, raw.x, raw.y, road_dist, snap_dist,
                            drag->x, drag->y);
                    } else {
                        dbg(lvl_debug, "road_gap: cursor_map=(%d,%d) closest=(%d,%d) road_dist=%.1fm drag=(%d,%d)",
...
static void navit_animation_ensure_timer(struct navit *this_) {
    if (this_->ready == 3 && !this_->animation_timer) {
        this_->animation_timer = event_add_timeout(ANIMATION_TICK_MS, 1, this_->animation_cb);
    }
}
...
static int navit_animation_tick(void *data) {
    ...
    if (was_animating && this_->menu_was_active && !navit_gui_menu_active(this_)) {
        struct point cursor_fixed_menu;
        dbg(lvl_debug, "anim_tick: menu closed during animation, forcing redraw");
        this_->map_animating = 0;
        this_->menu_was_active = 0;
        navit_get_cursor_pnt(this_, &cursor_fixed_menu, 0, NULL);
        navit_set_center_cursor(this_, 0, 0);
        transform_copy(this_->trans, this_->trans_cursor);
        graphics_set_display_rotation(this_->gra, 0.0, cursor_fixed_menu.x, cursor_fixed_menu.y);
        navit_draw(this_);
    ...
    if (was_animating && navit_gui_menu_active(this_) && this_->gra) {
        struct point cursor_fixed_menu;
        navit_get_cursor_pnt(this_, &cursor_fixed_menu, 0, NULL);
        graphics_set_display_rotation(this_->gra, 0.0, cursor_fixed_menu.x, cursor_fixed_menu.y);
    }

    if (this_->follow_cursor && (primary_animating || (this_->vehicle && this_->vehicle->speed > 0.5)) && this_->vehicle
        && this_->gra && !navit_gui_menu_active(this_) && !this_->button_pressed) {
        struct navit_vehicle *nv = this_->vehicle;
        struct point offset;
        struct point cursor_fixed;
        struct coord predicted;
        struct coord *scroll_coord;
        int w, h, yaw, yaw_delta, yaw_inc_delta;
        {
            int target = nv->dir;
            int current = this_->anim_last_displayed_yaw;
        ...
        if (this_->tracking && tracking_get_predicted_coord(this_->tracking, &predicted))
            scroll_coord = &predicted;
        else
            scroll_coord = &nv->coord;
        {
            enum projection pro = transform_get_projection(this_->trans_cursor);
            struct point vehicle_screen;
            if (pro && transform_point(this_->trans_cursor, pro, scroll_coord, &vehicle_screen)) {
                struct point exact;
                exact.x = cursor_fixed.x - vehicle_screen.x;
                exact.y = cursor_fixed.y - vehicle_screen.y;
                vehicle_update_scroll_target(nv->vehicle, &exact);
            }
        }
        vehicle_get_mapdrag_offset(nv->vehicle, &offset);
        {
            ...
            if (margin_threshold > 0 && (abs_x > margin_threshold || abs_y > margin_threshold)) {
                dbg(lvl_debug, "scroll_tick: offset (%d,%d) > threshold %d, re-centering", offset.x, offset.y,
                    margin_threshold);
                navit_get_cursor_pnt(this_, &cursor_fixed, 0, NULL);
        ...
        if (offset.y < -2 * h)
            ...
            offset.y = 2 * h;
        dbg(lvl_debug, "scroll_tick: drag=(%d,%d) yaw=%d screen=%dx%d", offset.x, offset.y, yaw, w, h);
        {
            enum projection pro = transform_get_projection(this_->trans_cursor);
            if (pro) {
                struct point tp;
                struct coord under;
                tp.x = cursor_fixed.x - offset.x;
                tp.y = cursor_fixed.y - offset.y;
                if (transform_reverse(this_->trans_cursor, &tp, &under))
                    dbg(lvl_debug, "cursor_sees: vehicle=(%d,%d) under_cursor=(%d,%d) offset=(%d,%d)", nv->coord.x,
                        nv->coord.y, under.x, under.y, offset.x, offset.y);
            }
        }
        ...
        yaw_inc_delta = yaw - this_->anim_last_displayed_yaw;
        if (yaw_inc_delta < -180)
            yaw_inc_delta += 360;
        if (yaw_inc_delta > 180)
            yaw_inc_delta -= 360;

        if (yaw_delta > 15 || yaw_delta < -15) {
            dbg(lvl_debug, "scroll_tick: yaw_delta=%d immediate redraw", yaw_delta);
            transform_set_yaw(this_->trans, yaw);
            transform_set_yaw(this_->trans_cursor, yaw);
            navit_set_center_coord_screen(this_, scroll_coord, &cursor_fixed, 0);
            transform_copy(this_->trans, this_->trans_cursor);
        ...
        } else if (yaw_inc_delta > 1 || yaw_inc_delta < -1) {
            dbg(lvl_debug, "scroll_tick: yaw_delta=%d yaw_inc=%d display_rotation", yaw_delta, yaw_inc_delta);
            graphics_set_display_rotation(this_->gra, (double)yaw_delta, cursor_fixed.x, cursor_fixed.y);
            this_->anim_last_displayed_yaw = yaw;
            graphics_draw_mode(this_->gra, draw_mode_end);
        } else {
            graphics_draw_mode(this_->gra, draw_mode_end);
        ...
        struct point cursor_fixed2;
        this_->map_animating = 0;
        navit_get_cursor_pnt(this_, &cursor_fixed2, 0, NULL);
        {
            enum projection pro = transform_get_projection(this_->trans_cursor);
            if (pro) {
                struct coord under;
                if (transform_reverse(this_->trans_cursor, &cursor_fixed2, &under))
                    dbg(lvl_debug, "scroll_finish_PRE: vehicle=(%d,%d) cursor_at=(%d,%d)", this_->vehicle->coord.x,
                        this_->vehicle->coord.y, under.x, under.y);
            }
        }
        navit_set_center_cursor(this_, 0, 0);
        transform_copy(this_->trans, this_->trans_cursor);
        {
            enum projection pro = transform_get_projection(this_->trans_cursor);
            if (pro) {
                struct coord under_post;
                if (transform_reverse(this_->trans_cursor, &cursor_fixed2, &under_post))
                    dbg(lvl_debug, "scroll_finish_POST: vehicle=(%d,%d) cursor_sees=(%d,%d)", this_->vehicle->coord.x,
                        this_->vehicle->coord.y, under_post.x, under_post.y);
            }
        }
        ...
        graphics_set_display_rotation(this_->gra, 0.0, cursor_fixed2.x, cursor_fixed2.y);
        navit_draw(this_);
        vehicle_reset_map_scroll(this_->vehicle->vehicle);
        gettimeofday(&this_->scroll_finished_ts, NULL);
    }
...
void navit_set_timeout(struct navit *this_) {
    struct attr follow;
    dbg(lvl_debug, "navit_set_timeout: center_timeout=%d", this_->center_timeout);
...
int navit_handle_button(struct navit *this_, int pressed, int button, struct point *p,
    ...
    callback_list_call_attr_4(this_->attr_cbl, attr_button, this_, GINT_TO_POINTER(pressed), GINT_TO_POINTER(button),
                              p);
    if (this_->ignore_button) {
...
void navit_handle_motion(struct navit *this_, struct point *p) {
    ...
    if (this_->button_pressed && !this_->popped) {
        ...
        if (dx < -8 || dx > 8 || dy < -8 || dy > 8) {
            ...
            if (!this_->motion_timeout)
                this_->motion_timeout =
                    event_add_timeout(this_->drag_bitmap ? 10 : 100, 0, this_->motion_timeout_callback);
        }
    }
}
...
static void navit_window_roadbook_update(struct navit *this_) {
    ...
    if (mr) {
        ...
        while ((item = map_rect_get_item(mr))) {
            ...
            if (attr.u.num >= 2000) {
                param[1].value = g_strdup_printf(
                    "%5.1f %s", imperial == TRUE ? (float)attr.u.num * METERS_TO_MILES : (float)attr.u.num / 1000,
...
int navit_init(struct navit *this_) {
    ...
    if (this_->mapsets) {
        ...
        msh = mapset_open(ms);
        while (msh && (map = mapset_next(msh, 0))) {
            // pass new callback instance for each map in the mapset to make map callback list destruction work
            // correctly
...
void navit_zoom_to_rect(struct navit *this_, struct coord_rect *r) {
    ...
    while (scale < 1 << 20) {
        ...
        if (p1.x < 0 || p2.x < 0 || p1.x > w || p2.x > w || p1.y < 0 || p2.y < 0 || p1.y > h || p2.y > h)
            scale *= 2;
        else
            break;
    }
...
void navit_drag_map(struct navit *this_, struct point *origin, struct point *destination) {
    dbg(lvl_debug, "navit_drag_map: (%d,%d)->(%d,%d) anim=%d", origin->x, origin->y, destination->x, destination->y,
        this_->map_animating);
    if (this_->map_animating) {
        this_->map_animating = 0;
        if (this_->vehicle)
...
static int navit_set_attr_do(struct navit *this_, struct attr *attr, int init) {
    ...
    default:
        dbg(lvl_debug, "calling generic setter method for attribute type %s",
            attr_to_name(attr->type)) return navit_object_set_attr((struct navit_object *)this_, attr);
    }
    if (attr_updated && !init) {
 ...
 *
 * @param this_ The navit instance
 * @param layout The layout to set as default (if NULL, we will set the current layout according to the default name
 * stored in this_->default_layout_name
 *
 * @note If argument @p layout is NULL and the default layout name in the config file does not exist or has not been
 * provided in the config file, the default layout is unchanged
 */
    ...
    if (layout) {
        ...
        if (this_->default_layout_name) { /* If a default layout name was provided */
            ...
            if (default_layout) {
                ...
                dbg(lvl_warning, "No definition exists in config for specified default layout '%s'",
                    this_->default_layout_name);
            }
        }
    }
}
...
static void navit_vehicle_update_position(struct navit *this_, struct navit_vehicle *nv) {
    ...
    {
        struct attr attr_time;
        if (vehicle_get_attr(nv->vehicle, attr_position_time_iso8601, &attr_time, NULL))
            dbg(lvl_debug, "pos_update: nmea_time=%s raw=(%d,%d) speed=%d dir=%d", attr_time.u.str, nv->coord.x,
                nv->coord.y, nv->speed, nv->dir);
    }
    if (nv != this_->vehicle) {
        if (this_->ready == 3)
            navit_vehicle_draw(this_, nv, NULL);
    ...
    if (this_->ready == 3) {
        ...
        if (this_->button_pressed != 1 && this_->follow_cursor && (nv->follow == 0 || nv->follow_curr <= nv->follow)) {
            ...
            int new_yaw = nv->dir;
            dbg(lvl_debug, "pos_update: re-centering trans_yaw=%d nv_dir=%d", old_yaw, new_yaw);
            navit_get_cursor_pnt(this_, &cursor_screen, 0, NULL);
            if (this_->follow_cursor && tc && pro_old) {
                struct point p_at_new_yaw;
                int w, h;
                transform_get_size(this_->trans, &w, &h);
                transform_set_yaw(this_->trans, new_yaw);
                if (transform_point(this_->trans, pro_old, &nv->coord, &p_at_new_yaw)) {
                    transform_set_yaw(this_->trans, old_yaw);
                    {
                        struct point scroll_target = {cursor_screen.x - p_at_new_yaw.x,
                                                      cursor_screen.y - p_at_new_yaw.y};
                        ...
                        this_->scroll_last_fix = now;
                        {
                            ...
                            dbg(lvl_debug,
                                "pos_update: scroll_target=(%d,%d) yaw=%d->%d interval_ms=%d gap_from_finish=%d "
                                "screen=%dx%d",
                                scroll_target.x, scroll_target.y, old_yaw, new_yaw, interval_ms, gap_ms, w, h);
                            {
                                ...
                                if (dy < 0)
                                    dy = -dy;
                                mag = dx > dy ? dx : dy;
                                dbg(lvl_debug, "pos_update: scroll_magnitude=%d (%.1f%%)", mag,
                                    (double)mag * 100.0 / (w > h ? w : h));
                                if (mag < 10) {
                                    dbg(lvl_debug, "pos_update: SKIP_ANIM: mag=%d, re-centering now", mag);
                                    transform_set_yaw(this_->trans, new_yaw);
                                    transform_set_yaw(this_->trans_cursor, new_yaw);
                                    navit_set_center_cursor(this_, 0, 0);
                                    transform_copy(this_->trans, this_->trans_cursor);
                                    navit_vehicle_draw(this_, nv, &cursor_screen);
                                    goto pos_update_done;
                                }
                                {
                                    struct point smooth_target = scroll_target;
                                    if (mag > 1000) {
                                        smooth_target.x = (smooth_target.x * 0.3);
                                        smooth_target.y = (smooth_target.y * 0.3);
                                        dbg(lvl_debug, "pos_update: LARGE_JUMPS: smoothing %dx%d to %dx%d",
                                            scroll_target.x, scroll_target.y, smooth_target.x, smooth_target.y);
                                    } else if (mag > 500 && gap_ms > 2000) {
                                        smooth_target.x = (smooth_target.x * 0.1);
                                        smooth_target.y = (smooth_target.y * 0.1);
                                        dbg(lvl_debug, "pos_update: LONG_GAP: smoothing %dx%d to %dx%d",
                                            scroll_target.x, scroll_target.y, smooth_target.x, smooth_target.y);
                                    }
                                    if (smooth_target.x != scroll_target.x || smooth_target.y != scroll_target.y) {
                                        scroll_target = smooth_target;
                                    }
                                }
                            }
                        }
                        ...
                        {
                            enum projection pro2 = transform_get_projection(this_->trans_cursor);
                            if (pro2) {
                                struct point tp;
                                struct coord under;
                                transform_set_yaw(this_->trans, old_yaw);
                                tp.x = cursor_screen.x;
                                tp.y = cursor_screen.y;
                                if (transform_reverse(this_->trans, &tp, &under))
                                    dbg(lvl_debug,
                                        "scroll_start: vehicle=(%d,%d) cursor_at=(%d,%d) target=(%d,%d) "
                                        "interval=%d",
                                        nv->coord.x, nv->coord.y, under.x, under.y, scroll_target.x, scroll_target.y,
                                        interval_ms);
                            }
                        }
                        if (this_->map_animating) {
                            vehicle_update_scroll_target(this_->vehicle->vehicle, &scroll_target);
                        } else {
            ...
            dbg(lvl_debug, "pos_update: no scroll (button pressed or follow disabled), just drawing cursor");
            navit_vehicle_draw(this_, nv, NULL);
...
static int navit_add_vehicle(struct navit *this_, struct vehicle *v) {
    ...
    nv->callback.u.callback =
        callback_new_attr_2(callback_cast(navit_vehicle_update_position), attr_position_coord_geo, this_, nv);
    vehicle_add_attr(nv->vehicle, &nv->callback);
    nv->callback.u.callback = callback_new_attr_3(callback_cast(navit_vehicle_update_status), attr_position_fix_type,
                                                  this_, nv, attr_position_fix_type);
    vehicle_add_attr(nv->vehicle, &nv->callback);
    nv->callback.u.callback = callback_new_attr_3(callback_cast(navit_vehicle_update_status), attr_position_sats_used,
                                                  this_, nv, attr_position_sats_used);
    vehicle_add_attr(nv->vehicle, &nv->callback);
    nv->callback.u.callback = callback_new_attr_3(callback_cast(navit_vehicle_update_status), attr_position_hdop, this_,
                                                  nv, attr_position_hdop);
    vehicle_add_attr(nv->vehicle, &nv->callback);
...
void navit_layout_switch(struct navit *n) {
    ...
    if (l->dayname || l->nightname) {
        ...
        if (__sunriset__(year, month, day, geo_attr.u.coord_geo->lng, geo_attr.u.coord_geo->lat, n->sunrise_degrees, 1,
                         &trise, &tset)
            != 0) {
 ...
 */
static int navit_cmd_switch_layout_day_night(struct navit *this_, char *function, struct attr **in,
                                             struct attr ***out) {
...
void navit_destroy(struct navit *this_) {
    ...
    navit_lang_pref_free(this_->lang_pref);
