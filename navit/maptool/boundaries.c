    ...
    while ((ib = read_item(boundaries))) {
            ...
            if (iso) {
                ...
                if (!country)
                    osm_warning("relation", item_bin_get_relationid(ib), 0,
                                "Country Boundary contains unknown ISO3166-1 value '%s'\n", iso);
                else {
        ...
        while ((member = item_bin_get_attr(ib, attr_osm_member, member))) {
            ...
            if (member_type == rel_member_way) {
                ...
                relations_add_relation_member_entry(relations, relations_func, boundary, (gpointer)role, rel_member_way,
                                                    osm_id);
            }
...
static void boundary_compute_bbox(struct boundary *b) {
...
static void boundary_write_debug(struct boundary *b) {
    GList *sl;
    FILE *f, *fu = NULL;
    char *poly_name;
    ...
    poly_name = g_strdup_printf("country_%s_poly", b->iso2);
    ...
    while (sl) {
        ...
        if (!coord_is_equal(*gs->first, *gs->last)) {
            if (!fu) {
                char *broken_name = g_strdup_printf("country_%s_broken", b->iso2);
    ...
    if (fu) {
        char *broken_name;
        osm_warning("relation", item_bin_get_relationid(b->ib), 0, "Broken country polygon '%s'\n", b->iso2);
        fclose(fu);
        broken_name = g_strdup_printf("country_%s_broken", b->iso2);
...
GList *process_boundaries_finish(GList *boundaries_list) {
    ...
    while (l) {
        ...
        boundary_compute_bbox(boundary);
        boundary_write_debug(boundary);
