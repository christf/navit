...
#ifdef HAVE_LZMA
#    define LZMA_ARENA_MARGIN 65536
#    define LZMA_ALIGNMENT 16
...
static void *arena_alloc(void *opaque, size_t nmemb, size_t size) {
    ...
    if (nmemb && size > SIZE_MAX / nmemb)
        return NULL;
    total = nmemb * size;
    aligned = (total + LZMA_ALIGNMENT - 1) & ~((size_t)LZMA_ALIGNMENT - 1);
