...
#else
vehicle_gpsd_callback(struct gps_data_t *data, const char *buf, size_t len, int level)
#endif
{
...
#else
        if (priv->sats_used != data->satellites_used || priv->sats != data->satellites
            || priv->sats_signal != sats_signal) {
#endif
...
static void vehicle_gpsd_open(struct vehicle_priv *priv) {
    ...
    if (gpsbt_start(NULL, 0, 0, 0, errstr, sizeof(errstr), 0, &priv->context) < 0) {
        dbg(lvl_error, "Error connecting to GPS with gpsbt: (%d) %s (%s)", errno, strerror(errno), errstr);
    }
...
static struct vehicle_priv *vehicle_gpsd_new_gpsd(struct vehicle_methods *meth, struct callback_list *cbl,
                                                  struct attr **attrs) {
