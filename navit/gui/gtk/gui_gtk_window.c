...
static void gui_gtk_projections_init(struct gui_priv *this) {
    ...
    gui_gtk_add_radio_menu(this, "Projection mg", "Map & Guide", "/ui/MenuBar/Map/Projection/ProjectionMenuAdditions",
                           data, &this->projection_group);

    data = g_new(struct action_cb_data, 1);
    data->gui = this;
    data->attr.type = attr_projection;
    data->attr.u.projection = projection_garmin;
    gui_gtk_add_radio_menu(this, "Projection garmin", "Garmin", "/ui/MenuBar/Map/Projection/ProjectionMenuAdditions",
                           data, &this->projection_group);
}
...
static void gui_gtk_destinations_update(struct gui_priv *this) {
    ...
    if (navit_get_attr(this->nav, attr_former_destination_map, &attr, NULL) && attr.u.map
        ...
        while ((item = map_rect_get_item(mr))) {
            ...
            meninfo = g_new(struct gui_menu_info, 1);
            *meninfo = gui_gtk_add_menu(this, name, label,
                                        "/ui/MenuBar/Route/FormerDestinations/FormerDestinationMenuAdditions", 0, data);
...
static void gui_gtk_destinations_init(struct gui_priv *this) {
    navit_add_callback(this->nav,
                       callback_new_attr_1(callback_cast(gui_gtk_destinations_update), attr_destination, this));
...
static void gui_gtk_bookmarks_init(struct gui_priv *this) {
    ...
    bookmarks_add_callback(attr.u.bookmarks,
                           callback_new_attr_1(callback_cast(gui_gtk_bookmarks_update), attr_bookmark_map, this));
...
static struct gui_priv *gui_gtk_new(struct navit *nav, struct gui_methods *meth, struct attr **attrs, struct gui *gui) {
    ...
    *meth = gui_gtk_methods;
    ...
    gtk_window_add_accel_group(GTK_WINDOW(this->win), gtk_ui_manager_get_accel_group(this->ui_manager));
