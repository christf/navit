...
static void draw_aligned_osd_text(char *buffer, int align, struct osd_item *osd_item, struct graphics_gc *curr_color) {
    ...
    while (buffer) {
        ...
        graphics_get_text_bbox(osd_item->gr, osd_item->font, buffer, 0x10000, 0x0, p2, 0);
        switch (align & 12) {
        case 4:
            p.x = xspacing;
            break;
        ...
        p.y += height + yspacing;
        graphics_draw_text(osd_item->gr, curr_color ? curr_color : osd_item->graphic_fg_text, NULL, osd_item->font,
                           buffer, &p, 0x10000, 0);
        buffer = next;
...
static void osd_compass_draw(struct osd_priv_common *opc, struct navit *nav, struct vehicle *v) {
    ...
    if (v) {
        ...
        if (navit_get_attr(nav, attr_destination, &destination_attr, NULL)
            ...
            p.x = (opc->osd_item.w - bbox[2].x) / 2;
            p.y = opc->osd_item.h - opc->osd_item.h / 10;
            graphics_draw_text(opc->osd_item.gr, this->destination_dir_gc, NULL, opc->osd_item.font, buffer, &p,
                               0x10000, 0);
            g_free(buffer);
        }
    }
...
static void osd_compass_init(struct osd_priv_common *opc, struct navit *nav) {
    ...
    if (opc->osd_item.command)
        navit_add_callback(nav, this->click_cb =
...
static void osd_button_init(struct osd_priv_common *opc, struct navit *nav) {
    ...
    if (this->use_overlay) {
        ...
        graphics_add_callback(gra, this->draw_cb =
                                       callback_new_attr_2(callback_cast(osd_button_draw), attr_postdraw, opc, nav));
    }
...
static struct osd_priv *osd_button_new(struct navit *nav, struct osd_methods *meth, struct attr **attrs) {
    ...
    navit_add_callback(nav, this->navit_init_cb =
                                callback_new_attr_1(callback_cast(osd_button_init), attr_graphics_ready, opc));
...
static struct osd_priv *osd_image_new(struct navit *nav, struct osd_methods *meth, struct attr **attrs) {
    ...
    navit_add_callback(nav, this->navit_init_cb =
                                callback_new_attr_1(callback_cast(osd_image_init), attr_graphics_ready, opc));
 ...
 *
 * This function is registered as a callback function in {@link osd_navigation_status_new(struct navit *, struct
 * osd_methods *, struct attr **)}. It is called after graphics initialization has finished and can be used for any
 * initialization tasks which rely on a functional graphics system.
 *
 ...
 *
 * This initializes the data structures and registers {@link osd_navigation_status_init(struct osd_priv_common *, struct
 * navit *)} as a callback.
 *
...
struct osd_speed_warner {
    ...
    enum osd_speed_warner_eAnnounceState announce_state;
    int bTextOnly;
    struct graphics_image *img_active, *img_passive, *img_off, *img_current, *img_condition, *img_hazard, *img_wet,
        *img_danger, *img_active_condition;
    char *label_str;
    int timeout;
...
static void osd_cond_speed_warner_draw(struct osd_priv_common *opc, struct navit *navit, struct vehicle *v) {
    ...
    if (tracking && this->active) {
        ...
        struct attr maxspeed_attr, maxspeed_fwd_attr, maxspeed_bwd_attr, speed_attr, imperial_attr, direction_attr,
            maxspeed_cond_attr, maxspeed_fwd_cond_attr, maxspeed_bwd_cond_attr;
            ...
            if (direction >= 0) {
                ...
                if (cond_fwd_speed_set) {
                    ...
                    if (routespeed > cond_fwd_speed) {
                        ...
                        if (tracking_get_attr(tracking, attr_maxspeed_fwd_conditional_condition,
                                              &maxspeed_fwd_cond_attr, NULL)) {
                ...
                if (tracking_get_attr(tracking, attr_maxspeed_bwd_conditional_condition, &maxspeed_bwd_cond_attr,
                                      NULL)) {
    ...
    if (strstr(condition, "hazmat")) {
        ...
        if (attr.u.attrs) {
            if (vehicleprofile_get_attr((struct vehicleprofile *)attr.u.attrs, attr_vehicle_dangerous_goods, &attr,
                                        NULL)) {
                if (attr.u.num > 0)
    ...
    if (this->img_active && this->img_passive && this->img_off && this->img_active_condition) {
        ...
        pc.y = img->height + 5;
        graphics_draw_image(opc->osd_item.gr, opc->osd_item.graphic_bg, &pc,
                            this->active > 0 ? this->img_active_condition : this->img_off);
...
static void osd_cond_speed_warner_init(struct osd_priv_common *opc, struct navit *nav) {
    ...
    osd_set_std_graphic(nav, &opc->osd_item, (struct osd_priv *)opc);
    navit_add_callback(nav,
                       callback_new_attr_1(callback_cast(osd_cond_speed_warner_draw), attr_position_coord_geo, opc));
    navit_add_callback(nav, this->click_cb =
                                callback_new_attr_1(callback_cast(osd_cond_speed_warner_click), attr_button, opc));
 ...
 * @param format A string specifying how to format the attribute. Allowed format strings depend on the attribute; this
 * member can be NULL.
