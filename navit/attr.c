 ...
 * @return The corresponding {@code attr_type}, or {@code attr_none} if the string specifies a nonexistent or invalid
 * attribute type.
 */
...
struct attr *attr_new_from_text(const char *name, const char *value) {
    ...
    default:
        ...
        if (attr >= attr_type_int_begin && attr < attr_type_rel_abs_begin) {
            ...
            if (*tail) {
                dbg(lvl_error,
                    "Incorrect value '%s' for attribute '%s';  expected a number. "
                    "Defaulting to 0.\n",
                    value, name);
                ret->u.num = 0;
            }
        ...
        if (attr >= attr_type_rel_abs_begin && attr < attr_type_boolean_begin) {
            ...
            if (*tail) {
                if (!strcmp(tail, "%")) {
                    ...
                    dbg(lvl_error,
                        "Incorrect value '%s' for attribute '%s';  expected a number or a relative value in percent. "
                        "Defaulting to 0.\n",
                        value, name);
                    ret->u.num = 0;
                }
        ...
        if (attr >= attr_type_boolean_begin && attr <= attr_type_int_end) {
            if (!(g_ascii_strcasecmp(value, "no") && g_ascii_strcasecmp(value, "0")
                  && g_ascii_strcasecmp(value, "false")))
                ret->u.num = 0;
            else if (!(g_ascii_strcasecmp(value, "yes") && g_ascii_strcasecmp(value, "1")
                       && g_ascii_strcasecmp(value, "true")))
                ret->u.num = 1;
            else {
                dbg(lvl_error,
                    "Incorrect value '%s' for attribute '%s';  expected a boolean (no/0/false or yes/1/true). "
                    "Defaulting to 'true'.\n",
                    value, name);
                ret->u.num = 1;
            }
...
static char *flags_to_text(int flags) {
    ...
    if (flags & AF_ONEWAY)
        ret = g_strconcat_printf(ret, "%sAF_ONEWAY", ret ? "|" : "");
    if (flags & AF_ONEWAYREV)
        ret = g_strconcat_printf(ret, "%sAF_ONEWAYREV", ret ? "|" : "");
    if (flags & AF_SEGMENTED)
        ret = g_strconcat_printf(ret, "%sAF_SEGMENTED", ret ? "|" : "");
    if (flags & AF_ROUNDABOUT)
        ret = g_strconcat_printf(ret, "%sAF_ROUNDABOUT", ret ? "|" : "");
    if (flags & AF_ROUNDABOUT_VALID)
        ret = g_strconcat_printf(ret, "%sAF_ROUNDABOUT_VALID", ret ? "|" : "");
    if (flags & AF_ONEWAY_EXCEPTION)
        ret = g_strconcat_printf(ret, "%sAF_ONEWAY_EXCEPTION", ret ? "|" : "");
    if (flags & AF_SPEED_LIMIT)
        ret = g_strconcat_printf(ret, "%sAF_SPEED_LIMIT", ret ? "|" : "");
    if (flags & AF_CONDITIONAL_SPEED_LIMIT)
        ret = g_strconcat_printf(ret, "%sAF_CONDITIONAL_SPEED_LIMIT", ret ? "|" : "");
    if (flags & AF_SIZE_OR_WEIGHT_LIMIT)
        ret = g_strconcat_printf(ret, "%sAF_SIZE_OR_WEIGHT_LIMIT", ret ? "|" : "");
    if (flags & AF_THROUGH_TRAFFIC_LIMIT)
        ret = g_strconcat_printf(ret, "%sAF_THROUGH_TRAFFIC_LIMIT", ret ? "|" : "");
    if (flags & AF_TOLL)
        ret = g_strconcat_printf(ret, "%sAF_TOLL", ret ? "|" : "");
    if (flags & AF_SEASONAL)
        ret = g_strconcat_printf(ret, "%sAF_SEASONAL", ret ? "|" : "");
    if (flags & AF_UNPAVED)
        ret = g_strconcat_printf(ret, "%sAF_UNPAVED", ret ? "|" : "");
    if (flags & AF_FORD)
        ret = g_strconcat_printf(ret, "%sAF_FORD", ret ? "|" : "");
    if (flags & AF_UNDERGROUND)
        ret = g_strconcat_printf(ret, "%sAF_UNDERGROUND", ret ? "|" : "");
    if (flags & AF_DANGEROUS_GOODS)
        ret = g_strconcat_printf(ret, "%sAF_DANGEROUS_GOODS", ret ? "|" : "");
    ...
    if (flags & AF_EMERGENCY_VEHICLES)
        ret = g_strconcat_printf(ret, "%sAF_EMERGENCY_VEHICLES", ret ? "|" : "");
    if (flags & AF_TRANSPORT_TRUCK)
        ret = g_strconcat_printf(ret, "%sAF_TRANSPORT_TRUCK", ret ? "|" : "");
    if (flags & AF_DELIVERY_TRUCK)
        ret = g_strconcat_printf(ret, "%sAF_DELIVERY_TRUCK", ret ? "|" : "");
    if (flags & AF_PUBLIC_BUS)
        ret = g_strconcat_printf(ret, "%sAF_PUBLIC_BUS", ret ? "|" : "");
    if (flags & AF_TAXI)
        ret = g_strconcat_printf(ret, "%sAF_TAXI", ret ? "|" : "");
    if (flags & AF_HIGH_OCCUPANCY_CAR)
        ret = g_strconcat_printf(ret, "%sAF_HIGH_OCCUPANCY_CAR", ret ? "|" : "");
    if (flags & AF_CAR)
        ret = g_strconcat_printf(ret, "%sAF_CAR", ret ? "|" : "");
    if (flags & AF_MOTORCYCLE)
        ret = g_strconcat_printf(ret, "%sAF_MOTORCYCLE", ret ? "|" : "");
    if (flags & AF_MOPED)
        ret = g_strconcat_printf(ret, "%sAF_MOPED", ret ? "|" : "");
    if (flags & AF_HORSE)
        ret = g_strconcat_printf(ret, "%sAF_HORSE", ret ? "|" : "");
    if (flags & AF_BIKE)
        ret = g_strconcat_printf(ret, "%sAF_BIKE", ret ? "|" : "");
    if (flags & AF_PEDESTRIAN)
        ret = g_strconcat_printf(ret, "%sAF_PEDESTRIAN", ret ? "|" : "");
    return ret;
}
...
char *attr_to_text_ext(struct attr *attr, char *sep, enum attr_format fmt, enum attr_format def_fmt, struct map *map) {
    ...
    if (type >= attr_type_color_begin && type <= attr_type_color_end) {
        if (attr->u.color->a != 65535)
            return g_strdup_printf("#%02x%02x%02x%02x", attr->u.color->r >> 8, attr->u.color->g >> 8,
                                   attr->u.color->b >> 8, attr->u.color->a >> 8);
        else
            return g_strdup_printf("#%02x%02x%02x", attr->u.color->r >> 8, attr->u.color->g >> 8,
                                   attr->u.color->b >> 8);
    }
...
int attr_data_size(struct attr *attr) {
    ...
    if (attr->type == attr_poly_hole) {
        return (sizeof(attr->u.poly_hole->coord_count)
                + (attr->u.poly_hole->coord_count * sizeof(*attr->u.poly_hole->coord)));
    }
 ...
 * @param[in,out] pos As input, if pointer is non-NULL, this argument contains the character index inside @p line from
 * which to start the search (see description)
 * @param[out] val_ret Points to a buffer which will receive the value as text
 * @param[out] name_ret Points to a buffer which will receive the actual name of the attribute found in the line, if
 * NULL this argument won't be used. Note that the buffer provided here should be long enough to contain the attribute
 * name + a terminating NUL character
 *
...
int attr_from_line(const char *line, const char *name, int *pos, char *val_ret, char *name_ret) {
    ...
    for (;;) {
        ...
        while (*p) {
            ...
            if (*p == '\\') { /* Next character is escaped */
                ...
                if (*(p + 1)) /* Make sure the string is not terminating just after this escape character */
                    p++; /* if the string continues, skip the next character, whatever is is (space, double-quote or
                            backslash) */
                else
        ...
        if (name == NULL || (e - n == len && strncmp(n, name, len) == 0)) { /* We matched the searched attribute name */
            ...
            /* Note: in the strncpy* calls below, we give a max size_t argument exactly matching the string lengh we
             * want to copy within the source string e, so no terminating NUL char will be appended */
            if (escaped)
                strncpy_unescape(val_ret, e, len - escaped); /* Unescape if necessary */
            else
                strncpy(val_ret, e, len);
            /* Because no NUL terminating char was copied over, we manually append it here to terminate the C-string
             * properly, just after the copied string */
