...
static struct zip_eoc *binfile_read_eoc(struct file *fi) {
    ...
    if (eoc) {
        ...
        if (eoc->zipesig != zip_eoc_sig) {
            dbg(lvl_error, "map file %s: eoc signature check failed: 0x%x vs 0x%x", fi->name, eoc->zipesig,
                zip_eoc_sig);
...
static int binfile_search_cd(struct map_priv *m, int offset, char *name, int partial, int skip) {
    ...
    while (offset < end) {
        ...
        if (!m->search_data || m->search_offset > offset || offset - m->search_offset + sizeof(*cd) > m->search_size
...
#endif
        if (!skip && (partial || cd->zipcfnl == len) && !strncmp(cd->zipcfn, name, len))
            return offset;
        skip = 0;
    ...
    for (;;) {
        ...
        case 3:
            if (download_finish(download)) {
                ...
                callback_list_call_attr_0(m->cbl, attr_progress);
                if (async) {
                    push_zipfile_tile_do(download->mr, download->cd, download->zipfile, download->toffset,
                                         download->tlength);
                    ret = NULL;
                } else
                    ret = download->cd;
        ...
        case 7:
            g_free(m->progress);
            m->progress = g_strdup_printf("Download Map Directory %d%%",
                                          (int)(download->offset * 100 / download->zip64_eoc->zip64ecsz));
            callback_list_call_attr_0(m->cbl, attr_progress);
...
/**
 * @brief Test an item if it's duplicate. If it's not a duplicate, return new struct duplicate to be
 * duplicate_insert()'ed.
...
static struct item *binmap_search_get_item(struct map_search_priv *map_search) {
    ...
    for (;;) {
        while ((it = map_rect_get_item_binfile(map_search->mr))) {
            ...
            case attr_town_or_district_name:
                if (map_search->mr->tile_depth > 1 && item_is_town(*it)
                ...
                if (map_search->mr->tile_depth > 1 && item_is_town(*it)
                    && map_search->search.type != attr_district_name) {
                ...
                if (map_search->mr->tile_depth > 1 && item_is_district(*it)
                    && map_search->search.type != attr_town_name) {
            ...
            case attr_street_name:
                if (map_search->mode == 1) {
                    if (binfile_attr_get(it->priv_data, attr_street_name_match, &at)
                        ...
                        if (!linguistics_compare(at.u.str, map_search->search.u.str, mode)
                            && !duplicate(map_search, it, attr_street_name, 0)) {
                            return it;
                        }
                    }
                    continue;
                ...
                if (item_is_street(*it)) {
                    ...
                    if (binfile_attr_get(it->priv_data, attr_label, &at)) {
                        ...
                        if (linguistics_compare(at.u.str, map_search->search.u.str,
                                                mode | linguistics_cmp_expand | linguistics_cmp_words)) {
                ...
                if ((has_house_number || it->type == type_house_number_interpolation_even
                    ...
                    if (has_house_number) {
                        ...
                        if ((binfile_attr_get(it->priv_data, attr_street_name, &at2) || map_search->mode != 2)
                            ...
                            if (!(it->type == type_house_number_interpolation_even
                                  || it->type == type_house_number_interpolation_odd
                                  || it->type == type_house_number_interpolation_all))
                                return it;
                        }
                           ...
                           && binfile_attr_get(it->priv_data, attr_street_name, &at)
                           && !strcmp(at.u.str, map_search->parent_name)) {
                    /* If matching street segment found, prepare to expand house number search region +100m around each
                     * way point */
                    if (!(map_search->boundaries && !item_inside_poly_list(it, map_search->boundaries))) {
                        struct coord c;
        ...
        if (map_search->search.type == attr_house_number && map_search->mode == 2 && map_search->parent_name) {
            /* For unindexed house number search, check if street segments extending possible housenumber locations were
             * found */
            if (map_search->ms.u.c_rect.lu.x != map_search->rect_new.lu.x
                || map_search->ms.u.c_rect.lu.y != map_search->rect_new.lu.y
                || map_search->ms.u.c_rect.rl.x != map_search->rect_new.rl.x
                || map_search->ms.u.c_rect.rl.y != map_search->rect_new.rl.y) {
                ...
                dbg(lvl_debug, "Extended house number search region to %d x %d, restarting...",
                    map_search->ms.u.c_rect.rl.x - map_search->ms.u.c_rect.lu.x,
                    map_search->ms.u.c_rect.lu.y - map_search->ms.u.c_rect.rl.y);
                continue;
            }
        }
...
static int map_binfile_open(struct map_priv *m) {
    ...
    *magic = le32_to_cpu(*magic);
    ...
    if (mr) {
        ...
        if (m->map_version >= 16) {
            dbg(lvl_error,
                "%s: This map is incompatible with your navit version. Please update navit. (map version %d)",
