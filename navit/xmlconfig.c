        ...
        if (fixme) {
            ...
            while (attr_fixme[0]) {
                if (!strcmp(name, attr_fixme[0])) {
                    name = attr_fixme[1];
                    if (fixme_count++ < 10)
                        dbg(lvl_error, "Please change attribute '%s' to '%s' in <%s />", attr_fixme[0], attr_fixme[1],
                            fixme->element);
                    break;
                }
                attr_fixme += 2;
...
static const char *find_attribute(struct xmlstate *state, const char *attribute, int required) {
    ...
    if (required)
        g_set_error(state->error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
                    "element '%s' is missing attribute '%s'", state->element, attribute);
    return NULL;
}
...
static struct object_func object_funcs[] = {
    {attr_announcement, NEW(announcement_new), GET(announcement_get_attr), NULL, NULL, SET(announcement_set_attr),
     ADD(announcement_add_attr)},
...
static void start_element(xml_context *context, const gchar *element_name, const gchar **attribute_names,
                          const gchar **attribute_values, gpointer user_data, xmlerror **error) {
    ...
    if (!func) {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
                    "Element '%s' within unexpected context '%s'. Expected '%s'%s", element_name, parent_name,
                    possible_parents,
...
static void xinclude(xml_context *context, const gchar **attribute_names, const gchar **attribute_values,
    ...
    while (attribute_names[i]) {
        if (!g_ascii_strcasecmp("href", attribute_names[i])) {
            ...
            if (!doc_new.xpointer)
                ...
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
                            "xi:include has more than one xpointer");
                return;
            }
        } else {
    ...
    if (!href) {
        dbg(lvl_debug, "no href%s, using own ref '%s'", doc_new.xpointer ? " (but xpointer provided)" : "",
            doc_old->href);
        ...
        for (i = 0; i < count; i++) {
            ...
            if (*included_filename != '\0') {                     /* Non empty href */
                if (!g_path_is_absolute(included_filename)) {     /* The filename's path is relative */
                    ...
                    g_free(doc_base);
                    dbg(lvl_debug, "converted relative filename='%s' to absolute filename='%s'", we_files[i],
                        included_filename);
                }
...
static void xi_start_element(xml_context *context, const gchar *element_name, const gchar **attribute_names,
                             const gchar **attribute_values, gpointer user_data, xmlerror **error) {
    ...
    if (doc->active > 0 || xpointer_match(doc->xpointer, doc->first)) {
        ...
        start_element(context, element_name, xistate->attribute_names, xistate->attribute_values, doc->user_data,
                      error);
        doc->active++;
    }
}
...
static const GMarkupParser parser = {xi_start_element, xi_end_element, xi_text, NULL, NULL};
/**
