...
static GtkTreeModel *category_list_model(struct gtk_poi_search *search) {
    ...
    gtk_list_store_append(search->store_cat, &iter);
    gtk_list_store_set(search->store_cat, &iter, 0, geticon("restaurant.png"), 1, _("Restaurant"), 2, "poi_restaurant",
                       -1);
    gtk_list_store_append(search->store_cat, &iter);
    ...
    gtk_list_store_append(search->store_cat, &iter);
    gtk_list_store_set(search->store_cat, &iter, 0, geticon("parking.png"), 1, _("Car parking"), 2, "poi_car_parking",
                       -1);
    gtk_list_store_append(search->store_cat, &iter);
...
static GtkTreeModel *model_poi(struct gtk_poi_search *search) {
    ...
    sel = map_selection_rect_new(
        &pc, search_distance_meters * transform_scale(abs(center.y) + search_distance_meters * 1.5), 18);
        ...
        if (mr) {
            while ((item = map_rect_get_item(mr))) {
                ...
                if (item->type == selected && idist <= search_distance_meters) {
                    ...
                    gtk_list_store_set(search->store_poi, &iter, 0, direction, 1, idist, 2, g_strdup(label_attr.u.str),
                                       3, coord_item.x, 4, coord_item.y, -1);
                }
            }
            map_rect_destroy(mr);
        }
...
static void treeview_poi_changed(GtkWidget *widget, struct gtk_poi_search *search) {
    ...
    if (!path)
        return;
    if (!gtk_tree_model_get_iter(GTK_TREE_MODEL(search->store_cat_sorted), &iter, path))
        return;
...
static void button_destination_clicked(GtkWidget *widget, struct gtk_poi_search *search) {
    ...
    if (!path)
        return;
    if (!gtk_tree_model_get_iter(GTK_TREE_MODEL(search->store_poi_sorted), &iter, path))
        return;
...
void gtk_gui_poi(struct navit *nav) {
    ...
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(search->treeview_cat), -1, _("Category"), renderer,
                                                "text", 1, NULL);
    ...
    renderer = gtk_cell_renderer_text_new();
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(search->treeview_poi), -1, _("Direction"), renderer,
                                                "text", 0, NULL);
    renderer = gtk_cell_renderer_text_new();
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(search->treeview_poi), -1, _("Distance"), renderer,
                                                "text", 1, NULL);
    renderer = gtk_cell_renderer_text_new();
